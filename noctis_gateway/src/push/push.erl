...
-spec init([]) -> {ok, state()}.
init([]) ->
    PushEnabled = noctis_gateway_env:get(push_enabled),
    BaseState = #{
    ...
    case PushEnabled of
        true ->
            UgsMaxMb = noctis_gateway_env:get(push_user_guild_settings_cache_mb),
            PsMaxMb = noctis_gateway_env:get(push_subscriptions_cache_mb),
            BiMaxMb = noctis_gateway_env:get(push_blocked_ids_cache_mb),
            BcMaxMb = noctis_gateway_env:get(push_badge_counts_cache_mb),
            BcTtl = noctis_gateway_env:get(push_badge_counts_cache_ttl_seconds),
            {ok, BaseState#{
...
handle_message_create(Params) ->
    case noctis_gateway_env:get(push_enabled) of
        true ->
            logger:debug(
