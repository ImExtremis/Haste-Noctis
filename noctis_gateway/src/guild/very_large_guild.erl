...
determine_shard_count(GuildState) ->
    ...
    case Override of
        N when is_integer(N), N >= 1 ->
            N;
        _ ->
            EnvValue = noctis_gateway_env:get(very_large_guild_shards),
            case EnvValue of
                N when is_integer(N), N >= 1 ->
                    N;
                _ ->
...
push_notification_sent_once_test() ->
    ensure_test_deps(),
    PrevPushEnabled = noctis_gateway_env:get(push_enabled),
    _ = noctis_gateway_env:patch(#{push_enabled => true}),
    ...
    _ = noctis_gateway_env:patch(#{push_enabled => PrevPushEnabled}),
