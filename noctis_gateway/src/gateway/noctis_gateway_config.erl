...
-spec load() -> config().
load() ->
    case os:getenv("NOCTIS_CONFIG") of
        false -> erlang:error({missing_env, "NOCTIS_CONFIG"});
        "" -> erlang:error({missing_env, "NOCTIS_CONFIG"});
        Path -> load_from(Path)
    end.
...
build_config(Json) ->
    ...
    #{
        ...
        release_node => get_string(Service, <<"release_node">>, "noctis_gateway@127.0.0.1"),
        ...
        telemetry => #{
            ...
            service_name => get_string(Telemetry, <<"service_name">>, "noctis-gateway"),
