...
get_loaded_modules() ->
    [M || {M, _} <- code:all_loaded(), is_noctis_module(M)].
...
get_changed_modules() ->
    ...
    [M || M <- Modified, is_noctis_module(M), not is_critical_module(M)].

-spec is_critical_module(atom()) -> boolean().
is_critical_module(Module) ->
    lists:member(Module, ?CRITICAL_MODULES).

-spec is_noctis_module(atom()) -> boolean().
is_noctis_module(Module) ->
    ModuleStr = atom_to_list(Module),
    lists:prefix("noctis_", ModuleStr) orelse
