...
-module(noctis_gateway_env).

-export([load/0, get/1, get_optional/1, get_map/0, patch/1, update/1]).

-define(CONFIG_TERM_KEY, {noctis_gateway, runtime_config}).
...
set_opentelemetry_env(Telemetry, Sentry, Config) ->
    ...
    ServiceName = maps:get(service_name, Telemetry, "noctis-gateway"),
