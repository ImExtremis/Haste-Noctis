...
-include_lib("noctis_gateway/include/timeout_config.hrl").
...
determine_shard_count() ->
    case noctis_gateway_env:get(presence_shards) of
...
with_runtime_config(Key, Value, Fun) ->
    Original = noctis_gateway_env:get(Key),
    noctis_gateway_env:patch(#{Key => Value}),
    Result = Fun(),
    noctis_gateway_env:update(fun(Map) ->
        case Original of
