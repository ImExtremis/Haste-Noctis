...
const GATEWAY_PATH = '/opt/noctis_gateway/bin/noctis_gateway';
...
function generateUniqueNodeName(): string {
	...
	return `noctis_gateway_${timestamp}_${uniqueId}@127.0.0.1`;
}

function killStaleGatewayProcesses(logger: Logger): void {
	const result = spawnSync('pgrep', ['-f', 'noctis_gateway.*@'], {timeout: 1000, encoding: 'utf-8'});
...
export function createGatewayProcessManager(): GatewayProcessManager {
	...
	const start = async (): Promise<void> => {
		...
		logger.info({nodeName: currentNodeName}, 'Starting Noctis Gateway process with unique node name');

		const gatewayEnv = {
			...process.env,
			NOCTIS_GATEWAY_HOST: Config.services.gateway.port ? '0.0.0.0' : '127.0.0.1',
			NOCTIS_GATEWAY_PORT: Config.services.gateway.port.toString(),
			NOCTIS_GATEWAY_NODE_FLAG: '-name',
			NOCTIS_GATEWAY_NODE_NAME: currentNodeName,
