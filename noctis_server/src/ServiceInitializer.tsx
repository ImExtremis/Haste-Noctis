...
import {type AdminAppResult, createAdminApp} from '@noctis/admin/src/App';
import type {AdminConfig} from '@noctis/admin/src/types/Config';
import {type APIAppResult, createAPIApp} from '@noctis/api/src/App';
import {buildAPIConfigFromMaster, initializeConfig} from '@noctis/api/src/Config';
import {DirectMediaService} from '@noctis/api/src/infrastructure/DirectMediaService';
...
import {createTestHarnessResetHandler, registerTestHarnessReset} from '@noctis/api/src/test/TestHarnessReset';
...
import {createAppServer} from '@noctis/app_proxy/src/AppServer';
import type {AppServerResult} from '@noctis/app_proxy/src/AppServerTypes';
...
import type {MediaProxyAppResult} from '@noctis/media_proxy/src/App';
import {createMediaProxyApp} from '@noctis/media_proxy/src/App';
import {JetStreamConnectionManager} from '@noctis/nats/src/JetStreamConnectionManager';
import type {S3AppResult} from '@noctis/s3/src/App';
import {createS3App} from '@noctis/s3/src/App';
...
function createS3Initializer(context: ServiceInitializationContext): ServiceInitializer {
	...
	const telemetry = createServiceTelemetry({serviceName: 'noctis-s3', skipPaths: ['/_health']});
...
async function createMediaProxyInitializer(
	...
	const telemetry = createServiceTelemetry({
		serviceName: 'noctis-media-proxy',
...
function createAppServerInitializer(context: ServiceInitializationContext): ServiceInitializer {
	...
	const telemetry = createServiceTelemetry({serviceName: 'noctis-app', skipPaths: ['/_health']});
