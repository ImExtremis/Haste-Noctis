...
export async function mountRoutes(options: MountRoutesOptions): Promise<MountedRoutes> {
	...
	applyMiddlewareStack(app, {
		...
		// Phase F: Security headers on every response
		securityHeaders: {
			// HSTS is the responsibility of the reverse proxy in production;
			// we set it here too for defense-in-depth.
			hsts: process.env.NODE_ENV === 'production',
			hstsMaxAge: 63_072_000, // 2 years
			noSniff: true,
			noFrame: true,
			referrerPolicy: true,
			// The SPA itself needs a permissive CSP set by the app_proxy; this
			// noctis_server instance only handles API + admin sub-routes so we
			// keep a strict API-safe default here.
	...
	try {
		...
		if (services.api !== undefined) {
			...
			app.get('/.well-known/noctis', (ctx) => apiService.app.fetch(ctx.req.raw));
