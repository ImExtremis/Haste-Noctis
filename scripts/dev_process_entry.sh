...
lock_dir="${runtime_dir}/noctis_dev_bootstrap.lock"
stamp_file="${runtime_dir}/noctis_dev_bootstrap.done"

mkdir -p "$repo_root/dev/logs"

if [ "${NOCTIS_SKIP_BOOTSTRAP:-0}" != "1" ]; then
	if [ "${NOCTIS_FORCE_BOOTSTRAP:-0}" = "1" ] || [ ! -f "$stamp_file" ]; then
