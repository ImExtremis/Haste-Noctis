...
ensure_core_secrets() {
    ...
    seeded_s3_access_key_id=$(seed_hex_secret "$current_s3_access_key_id" 16 "dev-access-key" "noctis-dev-access-key")
    seeded_s3_secret_access_key=$(seed_hex_secret "$current_s3_secret_access_key" 32 "dev-secret-key" "noctis-dev-secret-key")
...
generate_bluesky_oauth_keys() {
    ...
    jq --arg kid "dev-key-1" \
       ...
       '.auth.bluesky.logo_uri = "https://static.noctis.app/web/apple-touch-icon.png" |
        .auth.bluesky.tos_uri = "https://noctis.app/terms" |
        .auth.bluesky.policy_uri = "https://noctis.app/privacy" |
...
setup_model_symlink() {
    source="$REPO_ROOT/noctis_media_proxy/data/model.onnx"
    target_dir="$REPO_ROOT/noctis_server/data"
...
main() {
    echo ""
    info "Noctis Development Bootstrap"
    echo ""
