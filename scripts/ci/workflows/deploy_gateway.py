...
CONTAINER_ID="$(ssh "${SERVER}" "docker ps -q --filter label=com.docker.swarm.service.name=noctis-gateway_app | head -1")"
if [ -z "${CONTAINER_ID}" ]; then
  echo "::error::No running container found for service noctis-gateway_app"
  ssh "${SERVER}" "docker ps --filter 'name=noctis-gateway_app' --format '{{.ID}} {{.Names}} {{.Status}}'" || true
  exit 1
fi
...
LOCAL_MD5_LINES="$(
  erl -noshell -eval '
    Files = filelib:wildcard("noctis_gateway/_build/prod/lib/noctis_gateway/ebin/*.beam"),
    lists:foreach(
      fun(F) ->
...
REMOTE_MD5_LINES="$(
  ssh "${SERVER}" "docker exec ${CONTAINER_ID} /opt/noctis_gateway/bin/noctis_gateway eval '
