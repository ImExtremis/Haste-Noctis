...
""",
    "clone_target": """
set -euo pipefail
git clone --depth 1 "https://x-access-token:${TOKEN}@github.com/noctisapp/noctis_desktop.git" target || {
  mkdir target
  cd target
  git init
  git remote add origin "https://x-access-token:${TOKEN}@github.com/noctisapp/noctis_desktop.git"
}
""",
    "configure_git": """
set -euo pipefail
...
cp -a source/noctis_desktop/. target/
""",
    "commit_and_push": """
set -euo pipefail
cd target
...
git commit -m "Sync from noctisapp/noctis @ ${SOURCE_SHA}"
git push origin "HEAD:refs/heads/${BRANCH}"

echo "Synced to noctisapp/noctis_desktop:${BRANCH}"
""",
    "summary": """
set -euo pipefail
{
  ...
  echo ""
  echo "- **Source:** \`noctisapp/noctis:${BRANCH_NAME}\`"
  echo "- **Destination:** \`noctisapp/noctis_desktop:${BRANCH_NAME}\`"
