...
""",
    ...
    + """
rclone copy noctis_app/dist/assets ovh:noctis-static/assets \
  --transfers 32 \
  --checkers 16 \
  --size-only \
  --fast-list \
...
EOF
if [[ "${RELEASE_CHANNEL}" == "canary" ]]; then
  API_TARGET="noctis-api-canary_app"
else
  API_TARGET="noctis-api_app"
fi
