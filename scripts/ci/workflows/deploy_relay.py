...
CONTAINER_ID="$(ssh "${SERVER}" "docker ps -q --filter label=com.docker.swarm.service.name=noctis_relay_app | head -1")"
if [ -z "${CONTAINER_ID}" ]; then
  echo "::error::No running container found for service noctis_relay_app"
  ssh "${SERVER}" "docker ps --filter 'name=noctis_relay_app' --format '{{.ID}} {{.Names}} {{.Status}}'" || true
  exit 1
fi
...
LOCAL_MD5_LINES="$(
  erl -noshell -eval '
    Files = filelib:wildcard("noctis_relay/_build/prod/lib/noctis_relay/ebin/*.beam"),
    lists:foreach(
      fun(F) ->
...
REMOTE_MD5_LINES="$(
  ssh "${SERVER}" "docker exec ${CONTAINER_ID} /opt/noctis_relay/bin/noctis_relay eval '
