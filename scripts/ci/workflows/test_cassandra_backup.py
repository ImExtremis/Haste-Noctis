...
LATEST_BACKUP="$(
  rclone lsf "B2S3:noctis" --recursive --files-only --fast-list \
...
rclone copyto "B2S3:noctis/${LATEST_BACKUP}" "${WORKDIR}/backup.tar.age" --fast-list
...
for i in $(seq 1 20); do
  USER_COUNT="$(
    docker exec "${CASS_CONTAINER}" cqlsh -e "SELECT COUNT(*) FROM noctis.users;" 2>/dev/null \
