...
expected="app.noctis"
if [[ "${BUILD_CHANNEL:-stable}" == "canary" ]]; then expected="app.noctis.canary"; fi
...
$lines = @(
  ...
  'prefix = os.path.join(os.environ["LOCALAPPDATA"], "noctis_app", f"app-{build_ver}", "resources", "app.asar.unpacked")'
...
$dist = Join-Path $env:WORKDIR "noctis_desktop/dist-electron"
...
DIST="${WORKDIR}/noctis_desktop/dist-electron"
...
shopt -s nullglob
for dir in artifacts/noctis-desktop-${CHANNEL}-*; do
  [ -d "$dir" ] || continue

  base="$(basename "$dir")"
  if [[ "$base" =~ ^noctis-desktop-[a-z]+-([a-z]+)-([a-z0-9]+)$ ]]; then
