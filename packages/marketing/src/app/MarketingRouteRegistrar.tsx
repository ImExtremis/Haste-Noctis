...
import {CdnEndpoints} from '@noctis/constants/src/CdnEndpoints';
import {Headers, HeaderValues} from '@noctis/constants/src/Headers';
import {HttpStatus, MimeType} from '@noctis/constants/src/HttpConstants';
import {isPressAssetId, PressAssets} from '@noctis/constants/src/PressAssets';
import {createSession} from '@noctis/hono/src/Session';
import {getLocaleFromCode} from '@noctis/locale/src/LocaleService';
import type {MarketingConfig} from '@noctis/marketing/src/MarketingConfig';
import {sendMarketingRequest} from '@noctis/marketing/src/MarketingHttpClient';
import {renderCareersPage} from '@noctis/marketing/src/pages/CareersPage';
import {renderDonateManagePage} from '@noctis/marketing/src/pages/DonateManagePage';
import {renderDonatePage} from '@noctis/marketing/src/pages/DonatePage';
import {renderDonateSuccessPage} from '@noctis/marketing/src/pages/DonateSuccessPage';
import {renderDownloadPage} from '@noctis/marketing/src/pages/DownloadPage';
import {renderHelpArticlePage} from '@noctis/marketing/src/pages/HelpArticlePage';
import {renderHelpIndexPage} from '@noctis/marketing/src/pages/HelpIndexPage';
import {renderHomePage} from '@noctis/marketing/src/pages/HomePage';
import {renderNotFoundPage} from '@noctis/marketing/src/pages/NotFoundPage';
import {renderPartnersPage} from '@noctis/marketing/src/pages/PartnersPage';
import {renderStellarPage} from '@noctis/marketing/src/pages/StellarPage';
import {renderPolicyPage} from '@noctis/marketing/src/pages/PolicyPage';
import {renderPressPage} from '@noctis/marketing/src/pages/PressPage';
import {sanitizeInternalRedirectPath} from '@noctis/marketing/src/RedirectPathUtils';
import type {MarketingRouteHandler} from '@noctis/marketing/src/routes/RouteTypes';
import {generateSitemap} from '@noctis/marketing/src/Sitemap';
import {prependBasePath} from '@noctis/marketing/src/UrlUtils';
...
const PAGE_ROUTE_DEFINITIONS: ReadonlyArray<{
	...
	{path: '/stellar', handler: renderStellarPage},
...
function registerExternalRedirects(app: Hono): void {
	app.get('/get/livekitctl', (c) => {
		return c.redirect(
			'https://raw.githubusercontent.com/noctisapp/noctis/main/noctis_devops/livekitctl/scripts/install.sh',
			HttpStatus.FOUND,
		);
	});

	app.get('/regional-restrictions', (c) => c.redirect('/help/regional-restrictions', HttpStatus.MOVED_PERMANENTLY));
	app.get('/blog', (c) => c.redirect('https://blog.noctis.app', HttpStatus.FOUND));
	app.get('/blog/*', (c) => c.redirect('https://blog.noctis.app', HttpStatus.FOUND));
}
...
function registerSystemContentRoutes(app: Hono, contextFactory: MarketingContextFactory): void {
	...
	registerContextRoute(app, '/security.txt', contextFactory, (_c, ctx) => {
		...
		const body = [
			`Contact: ${securityUrl}`,
			'Contact: mailto:security@noctis.app',
			`Expires: ${expires}`,
