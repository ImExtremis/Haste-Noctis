...
import {defaultHeroPadding} from '@noctis/marketing/src/components/HeroBase';
...
import {getSystemRequirements, renderDesktopButton} from '@noctis/marketing/src/components/PlatformDownloadButton';
import {renderPwaInstallModal, renderPwaInstallTrigger} from '@noctis/marketing/src/components/PwaInstallDialog';
...
export async function renderDownloadPage(c: Context, ctx: MarketingContext): Promise<Response> {
	...
	const meta = pageMeta(
		ctx.i18n.getMessage('download.download_noctis', ctx.locale),
	...
	return (
		<section class={`flex flex-col items-center justify-center ${defaultHeroPadding()}`}>
			<div class="max-w-4xl space-y-8 text-center">
				...
				<h1 class="hero">{ctx.i18n.getMessage('download.download_noctis', ctx.locale)}</h1>
		...
		<section class={GRADIENTS.light}>
			...
			>
				<div class="flex flex-col gap-6 sm:flex-row sm:justify-center">
					...
					{renderSupportCard(
						'https://github.com/noctisapp/noctis',
