...
function renderHeroSection(ctx: MarketingContext): JSX.Element {
	return (
		<HeroBase
			icon={<NoctisPartnerIcon class="h-14 w-14 md:h-18 md:w-18" />}
			...
			<div class="mx-auto max-w-6xl">
				...
				<div class="grid gap-6 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
					{renderPerkCard(
						ctx,
						'noctis_premium',
						ctx.i18n.getMessage('partner_program.perks.free_stellar.label', ctx.locale),
						ctx.i18n.getMessage('partner_program.perks.free_stellar.description', ctx.locale),
						false,
						'/stellar',
					)}
					{renderPerkCard(
		...
		<section class={GRADIENTS.light}>
			<div class={`${GRADIENTS.cta} rounded-t-3xl`}>
				<div class={`mx-auto max-w-4xl ${SPACING.cta} text-center`}>
					...
					<a
						href="mailto:partners@noctis.app"
