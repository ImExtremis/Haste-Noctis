...
import {Section} from '@noctis/marketing/src/components/Section';
import {SupportCard} from '@noctis/marketing/src/components/SupportCard';
...
import {GRADIENTS} from '@noctis/ui/src/styles/Gradients';
import {SPACING} from '@noctis/ui/src/styles/Spacing';
...
export async function renderCareersPage(c: Context, ctx: MarketingContext): Promise<Response> {
	...
	const meta = pageMeta(
		ctx.i18n.getMessage('company_and_resources.careers.careers_at_noctis', ctx.locale),
		ctx.i18n.getMessage('misc_labels.join_noctis_community_contribute', ctx.locale),
		'website',
	);
...
function renderHeroSection(ctx: MarketingContext): JSX.Element {
	return (
		<HeroBase
			icon={<NoctisStaffIcon class="h-14 w-14 text-white md:h-18 md:w-18" />}
			title={ctx.i18n.getMessage('misc_labels.join_team_behind_noctis', ctx.locale)}
	...
	return (
		<Section
			...
			description={ctx.i18n.getMessage(
				'company_and_resources.source_and_contribution.noctis_built_in_open',
				ctx.locale,
			)}
		>
			<div class="grid grid-cols-1 gap-10 md:grid-cols-2 md:gap-12">
				<SupportCard
					...
					buttonHref="https://github.com/noctisapp/noctis"
				/>
				<SupportCard
					ctx={ctx}
					icon="translate"
					...
					buttonText="i18n@noctis.app"
					buttonHref="mailto:i18n@noctis.app"
				/>
				<SupportCard
					ctx={ctx}
					...
					buttonText={ctx.i18n.getMessage('misc_labels.join_noctis_hq', ctx.locale)}
					buttonHref="https://noctis.gg/noctis-hq"
				/>
				<SupportCard
					ctx={ctx}
					icon="bug"
				...
				<SupportCard
					...
					buttonText="safety@noctis.app"
					buttonHref="mailto:safety@noctis.app"
				/>
				<SupportCard
					ctx={ctx}
					icon="noctis_partner"
			...
			<div class="mx-auto max-w-7xl">
				<div class="mb-16 text-center md:mb-20">
					<h2 class="display mb-6 text-5xl text-black md:mb-8 md:text-6xl lg:text-7xl">
						{ctx.i18n.getMessage('company_and_resources.community_team.noctis_community_team', ctx.locale)}
					</h2>
				...
				<div class="mt-10 text-center md:mt-12">
					<a
						href="mailto:careers@noctis.app"
			...
			<div class={`${GRADIENTS.cta} rounded-t-3xl`}>
				<div class={`mx-auto max-w-4xl ${SPACING.cta} text-center`}>
					<h2 class="display mb-6 text-4xl md:mb-8 md:text-5xl lg:text-6xl">
						{ctx.i18n.getMessage('company_and_resources.source_and_contribution.want_to_help_build_noctis', ctx.locale)}
					</h2>
					...
					<div class="flex flex-col items-center justify-center gap-3 sm:flex-row sm:gap-4">
						...
						<a
							href="https://noctis.gg/noctis-hq"
							target="_blank"
						...
						>
							...
							<span>{ctx.i18n.getMessage('misc_labels.join_noctis_hq', ctx.locale)}</span>
						</a>
					</div>
				</div>
			</div>
