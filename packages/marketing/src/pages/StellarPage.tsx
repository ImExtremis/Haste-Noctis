...
import {
	...
	STELLAR_PERKS,
	type StellarPerk,
} from '@noctis/constants/src/StellarPerks';
import {FinalCtaSection} from '@noctis/marketing/src/components/FinalCtaSection';
import {defaultHeroPadding, HeroBase} from '@noctis/marketing/src/components/HeroBase';
import {CheckIcon} from '@noctis/marketing/src/components/icons/CheckIcon';
import {CrossIcon} from '@noctis/marketing/src/components/icons/CrossIcon';
import {NoctisPremiumIcon} from '@noctis/marketing/src/components/icons/NoctisPremiumIcon';
import {Icon} from '@noctis/marketing/src/components/icons/IconRegistry';
import type {MarketingContext} from '@noctis/marketing/src/MarketingContext';
import {Currency, getCurrency, getFormattedPrice, PricingTier} from '@noctis/marketing/src/PricingUtils';
import {renderLayout} from '@noctis/marketing/src/pages/Layout';
import {pageMeta} from '@noctis/marketing/src/pages/layout/Meta';
import {formatNumber} from '@noctis/marketing/src/utils/NumberFormatUtils';
import type {Context} from 'hono';

export async function renderStellarPage(c: Context, ctx: MarketingContext): Promise<Response> {
	...
	const meta = pageMeta(
		ctx.i18n.getMessage('pricing_and_tiers.stellar.tier_name', ctx.locale),
		ctx.i18n.getMessage('pricing_and_tiers.stellar.upgrade_pitch', ctx.locale),
		'website',
	);
...
function renderHeroSection(ctx: MarketingContext): JSX.Element {
	...
	return (
		<HeroBase
			icon={<NoctisPremiumIcon class="h-14 w-14 text-white md:h-18 md:w-18" fillColor="#4641D9" />}
			title={ctx.i18n.getMessage('pricing_and_tiers.stellar.tier_name', ctx.locale)}
			description={ctx.i18n.getMessage('pricing_and_tiers.stellar.higher_limits_and_early_access', ctx.locale)}
			extraContent={
				<div>
					<p class="body-lg mx-auto mt-6 mb-10 max-w-3xl text-white/70 md:mb-12">
						{ctx.i18n.getMessage('pricing_and_tiers.stellar.benefits_note_official_instance_only', ctx.locale)}
					</p>
...
function getPerkLabel(ctx: MarketingContext, perk: StellarPerk): string {
	const labelMap: Record<string, string> = {
		custom_4_digit_username_tag: ctx.i18n.getMessage(
			'pricing_and_tiers.stellar.features.custom_4_digit_username_tag',
			ctx.locale,
		),
		per_community_profiles: ctx.i18n.getMessage(
			'pricing_and_tiers.stellar.features.per_community_profiles',
			ctx.locale,
		),
		message_scheduling: ctx.i18n.getMessage('pricing_and_tiers.stellar.features.message_scheduling', ctx.locale),
		profile_badge: ctx.i18n.getMessage('misc_labels.profile_badge', ctx.locale),
		custom_video_backgrounds: ctx.i18n.getMessage(
			'pricing_and_tiers.stellar.features.custom_video_backgrounds',
			ctx.locale,
		),
		...
		message_character_limit: ctx.i18n.getMessage(
			'pricing_and_tiers.stellar.features.message_character_limit',
			ctx.locale,
		),
		bookmarked_messages: ctx.i18n.getMessage('app.messaging.features.bookmarked_messages', ctx.locale),
		file_upload_size: ctx.i18n.getMessage('pricing_and_tiers.stellar.features.file_upload_size', ctx.locale),
		emoji_sticker_packs: ctx.i18n.getMessage('pricing_and_tiers.stellar.features.emoji_sticker_packs', ctx.locale),
		saved_media: ctx.i18n.getMessage('pricing_and_tiers.stellar.features.saved_media', ctx.locale),
		...
		global_emoji_sticker_access: ctx.i18n.getMessage(
			'pricing_and_tiers.stellar.features.global_emoji_sticker_access',
			ctx.locale,
		),
...
function getStatusBadge(ctx: MarketingContext, perk: StellarPerk): string | null {
	if (perk.status === 'beta') {
...
function renderComparisonSection(ctx: MarketingContext): JSX.Element {
	...
	const renderPerkRow = (perk: StellarPerk): JSX.Element | null => {
		...
		if (isBooleanPerk(perk)) {
			...
			return comparisonCheckRow(ctx, label, perk.freeValue, perk.stellarValue, badge);
		}
		...
		if (isNumericPerk(perk)) {
			return comparisonRow(
				...
				formatPerkNumericValue(perk.stellarValue, perk.unit),
				badge,
			);
		}
			...
			<div class="mx-auto max-w-5xl">
				...
				<div class="overflow-x-auto">
					<table class="w-full border-collapse rounded-lg border border-gray-200" style="table-layout: fixed">
						...
						<tbody>{STELLAR_PERKS.map(renderPerkRow)}</tbody>
					</table>
				</div>
				<div class="mt-12 text-center md:mt-16">
					...
					>
						{ctx.i18n.getMessage('pricing_and_tiers.stellar.get_stellar', ctx.locale)}
					</a>
				</div>
			</div>
		</section>
...
function comparisonRow(
	...
	return (
		<tr class="border-gray-100 border-b">
			...
			<td class="label px-2 py-3 text-center text-[#4641D9] text-xs sm:px-3 sm:text-sm">{stellarValue}</td>
		</tr>
	);
}
	...
	return (
		<section class="bg-white px-6 py-24 sm:px-8 md:px-12 md:py-40 lg:px-16 xl:px-20">
			<div class="mx-auto max-w-7xl">
				<h2 class="display mb-16 text-center text-4xl text-black md:mb-20 md:text-5xl lg:text-6xl">
					{ctx.i18n.getMessage('pricing_and_tiers.stellar.get_more_with_stellar', ctx.locale)}
				</h2>
				<div class="grid grid-cols-1 gap-8 md:grid-cols-2 md:gap-10 lg:grid-cols-3">
					{renderFeatureCard(
						ctx,
						'hash',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.custom_username_tag', ctx.locale),
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.choose_custom_4_digit_tag', ctx.locale),
						null,
					)}
					{renderFeatureCard(
						ctx,
						'user_circle',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.per_community_profiles', ctx.locale),
					...
					{renderFeatureCard(
						ctx,
						'calendar_check',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.message_scheduling', ctx.locale),
					...
					{renderFeatureCard(
						ctx,
						'smiley',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.global_emoji_sticker_access', ctx.locale),
					...
					{renderFeatureCard(
						ctx,
						'video',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.features.custom_video_backgrounds', ctx.locale),
					...
					{renderFeatureCard(
						ctx,
						'arrow_up',
						ctx.i18n.getMessage('pricing_and_tiers.stellar.higher_limits_everywhere', ctx.locale),
						ctx.i18n.getMessage('pricing_and_tiers.stellar.feature_highlights', ctx.locale),
						null,
					)}
					{renderFeatureCard(
						ctx,
						...
						ctx.i18n.getMessage('pricing_and_tiers.stellar.show_off_status_badge', ctx.locale),
						null,
					)}
					{renderFeatureCard(
						ctx,
