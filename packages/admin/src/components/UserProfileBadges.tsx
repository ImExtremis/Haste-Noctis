...
import {hasBigIntFlag, parseBigIntOrZero} from '@noctis/admin/src/utils/Bigint';
...
import {getFormattedShortDate} from '@noctis/date_utils/src/DateFormatting';
import type {UserAdminResponse} from '@noctis/schema/src/domains/admin/AdminUserSchemas';
import {normalizeEndpoint} from '@noctis/ui/src/utils/AvatarMediaUtils';
...
export function UserProfileBadges({config, user, size = 'sm', class: className}: UserProfileBadgesProps) {
	...
	if (!isSelfHosted && hasBigIntFlag(flags, UserFlags.BUG_HUNTER)) {
		badges.push({
			...
			tooltip: 'Noctis Bug Hunter',
		});
	}

	if (!isSelfHosted && user.premium_type && user.premium_type !== UserPremiumTypes.NONE) {
		let tooltip = 'Noctis Stellar';

		if (user.premium_type === UserPremiumTypes.LIFETIME) {
			if (user.premium_since) {
				const premiumSince = getFormattedShortDate(user.premium_since);
				tooltip = `Noctis Visionary since ${premiumSince}`;
			} else {
				tooltip = 'Noctis Visionary';
			}
		} else if (user.premium_since) {
			const premiumSince = getFormattedShortDate(user.premium_since);
			tooltip = `Noctis Stellar subscriber since ${premiumSince}`;
		}

		badges.push({
			key: 'premium',
			iconUrl: `${staticCdnEndpoint}/badges/stellar.svg`,
			tooltip,
		});
	}
