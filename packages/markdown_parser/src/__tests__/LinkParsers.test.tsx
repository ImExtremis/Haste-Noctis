...
import type {LinkNode} from '@noctis/markdown_parser/src/types/Nodes';
...
describe('Noctis Markdown Parser', () => {
	...
	test('domain names without protocol should not be treated as links', () => {
		const input = '[free stellar](cakey.bot)';
		const flags = ParserFlags.ALLOW_MASKED_LINKS;
		const parser = new Parser(input, flags);
		const {nodes: ast} = parser.parse();

		expect(ast).toEqual([{type: NodeType.Text, content: '[free stellar](cakey.bot)'}]);
	});
