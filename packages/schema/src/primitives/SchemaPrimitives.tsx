...
export function withOpenApiType<T extends ZodTypeAny>(schema: T, typeName: string): T {
	(schema as Record<string, unknown>).__noctis_custom_type__ = typeName;
	return schema;
}
...
export const UnsignedInt64Type = z
	...
	.describe('noctis:UnsignedInt64Type');

export const Int64StringType = z
	.string()
	.regex(/^-?\d+$/)
	.describe('noctis:Int64StringType');

const SNOWFLAKE_REGEX = /^(0|[1-9][0-9]*)$/;
const UNSIGNED_INT64_STRING_REGEX = /^\d+$/;

export const SnowflakeStringType = z.string().regex(SNOWFLAKE_REGEX).describe('noctis:SnowflakeStringType');

export const BitflagStringType = z.string().regex(UNSIGNED_INT64_STRING_REGEX).describe('noctis:BitflagStringType');

const HEX_STRING_16_REGEX = /^[a-f0-9]{16}$/;

export const HexString16Type = z.string().regex(HEX_STRING_16_REGEX).describe('noctis:HexString16Type');

const HEX_STRING_32_REGEX = /^[a-f0-9]{32}$/;

export const HexString32Type = z.string().regex(HEX_STRING_32_REGEX).describe('noctis:HexString32Type');
...
export const ColorType = z
	...
	.describe('noctis:ColorType');

export const Int32Type = z.number().int().min(0).max(2147483647).describe('noctis:Int32Type');

export const SignedInt32Type = z.number().int().min(-2147483648).max(2147483647).describe('noctis:SignedInt32Type');
...
export function createNamedLiteral<T extends number>(value: T, name: string, description?: string) {
	...
	return z.literal(value).describe(`noctis:EnumValue:${JSON.stringify(entry)}`);
}
...
export function createNamedLiteralUnion<T extends number>(
	...
	return z
		...
		.describe(`noctis:EnumValues:${JSON.stringify(entries)}${descPart}`);
}
...
export function createFlexibleStringLiteralUnion<T extends string>(
	...
	return z.union(flexibleUnionOperands).describe(`noctis:FlexibleEnumValues:${JSON.stringify(entries)}${descPart}`);
}
...
export function createInt32EnumType<T extends number>(
	...
	const descPart = description ? ` ${description}` : '';
	return Int32Type.describe(`noctis:Int32Enum${typeNamePart}:${JSON.stringify(entries)}${descPart}`);
}
...
export function createBitflagStringType<T extends BitflagConstantsObject>(
	...
	return BitflagStringType.describe(`noctis:Bitflags64${typeNamePart}:${annotation}${descPart}`);
}
...
export function createBitflagInt32Type<T extends BitflagConstantsObject>(
	...
	const typeNamePart = typeName ? `:${typeName}` : '';
	const descPart = overallDescription ? ` ${overallDescription}` : '';
	return Int32Type.describe(`noctis:Bitflags32${typeNamePart}:${annotation}${descPart}`);
}
...
export function createPermissionStringType<T extends BitflagConstantsObject>(
	...
	return z
		...
		.describe(`noctis:Permissions${typeNamePart}:${annotation}${descPart}`);
}
