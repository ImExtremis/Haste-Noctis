...
import {EmailType} from '@noctis/schema/src/primitives/UserValidators';
import {z} from 'zod';
...
const NOCTIS_TAG_REGEX = /^([^#]{1,32})#([0-9]{4})$/;

const NOCTIS_TAG_TYPE = z
	.string()
	.min(3)
	.max(37)
	.refine((value) => NOCTIS_TAG_REGEX.test(value), 'Noctis tag must be in the format username#1234')
	.describe('A Noctis username tag in the format username#1234');
...
const DsaReportBase = z.object({
	...
	reporter_noctis_tag: z.optional(NOCTIS_TAG_TYPE).describe('Noctis tag of the reporter if they have an account'),
});
...
export const DsaReportMessageRequest = DsaReportBase.extend({
	...
	reported_user_tag: z.optional(NOCTIS_TAG_TYPE).describe('Noctis tag of the user who sent the message'),
});
...
export const DsaReportUserRequest = DsaReportBase.extend({
	...
	user_tag: z.optional(NOCTIS_TAG_TYPE).describe('Noctis tag of the user being reported'),
