...
import {createErrorHandler, type ErrorHandlerOptions} from '@noctis/errors/src/ErrorHandler';
...
describe('createErrorHandler', () => {
	describe('NoctisError handling', () => {
		it('should return NoctisError response directly', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new NoctisError({
					code: 'TEST_ERROR',
		...
		it('should include NoctisError data in response', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new NoctisError({
		...
		it('should include NoctisError custom headers', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new NoctisError({
					code: 'RATE_LIMITED',
	...
	describe('logError callback', () => {
		...
		it('should call logError for NoctisError', async () => {
			const logError = vi.fn();
			const app = createTestApp({logError});
			app.get('/test', () => {
				throw new NoctisError({code: 'TEST', status: 400});
			});

			await app.request('/test');

			expect(logError).toHaveBeenCalledTimes(1);
			expect(logError.mock.calls[0][0]).toBeInstanceOf(NoctisError);
		});
	});
	...
	describe('customHandler callback', () => {
		...
		it('should fall back to default handling when customHandler returns undefined', async () => {
			...
			const app = createTestApp({customHandler});
			app.get('/test', () => {
				throw new NoctisError({code: 'FALLBACK', status: 400});
			});
