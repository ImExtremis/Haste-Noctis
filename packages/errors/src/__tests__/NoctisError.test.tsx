...
describe('NoctisError', () => {
	describe('constructor', () => {
		it('should create an error with required options', () => {
			...
			expect(error.name).toBe('NoctisError');
		});
	...
	describe('status codes', () => {
		it('should handle 4xx client error status codes', () => {
			...
			for (const {status} of testCases) {
				const error = new NoctisError({code: 'TEST', status});
	...
	describe('error properties', () => {
		it('should have correct name property', () => {
			const error = new NoctisError({code: 'TEST', status: 400});
			expect(error.name).toBe('NoctisError');
		});

		it('should be throwable', () => {
			const error = new NoctisError({code: 'THROWN_ERROR', status: 400});
		...
		it('should be catchable as Error', () => {
			const error = new NoctisError({code: 'CAUGHT_ERROR', status: 400});

			try {
				throw error;
			} catch (e) {
				expect(e).toBeInstanceOf(Error);
				expect(e).toBeInstanceOf(NoctisError);
			}
		});
	});
});
