...
import {
	...
	UsernameTypeRef,
} from '@noctis/openapi/src/converters/BuiltInSchemas';
import {CustomSchemaType} from '@noctis/openapi/src/schemas/CustomSchemaType';
import type {OpenAPISchema, OpenAPISchemaOrRef} from '@noctis/openapi/src/Types';
import type {ZodTypeAny} from 'zod';

const NOCTIS_SCHEMA_NAME = Symbol('noctis.openapi.schemaName');
const NOCTIS_CUSTOM_TYPE_KEY = '__noctis_custom_type__';
...
export function setSchemaName(schema: ZodTypeAny, name: string): void {
	(schema as {[NOCTIS_SCHEMA_NAME]?: string})[NOCTIS_SCHEMA_NAME] = name;
}

function getSchemaName(schema: ZodTypeAny): string | undefined {
	return (schema as {[NOCTIS_SCHEMA_NAME]?: string})[NOCTIS_SCHEMA_NAME];
}

export function setCustomType(schema: ZodTypeAny, typeName: string): ZodTypeAny {
	(schema as unknown as Record<string, unknown>)[NOCTIS_CUSTOM_TYPE_KEY] = typeName;
	return schema;
}

function getCustomType(schema: ZodTypeAny, depth = 0): string | undefined {
	if (depth > 20) return undefined;
	const direct = (schema as unknown as Record<string, unknown>)[NOCTIS_CUSTOM_TYPE_KEY];
...
interface NoctisTypeAnnotation {
	typeName: string;
...
function parseNoctisTypeAnnotation(description: string | undefined): NoctisTypeAnnotation | null {
	if (!description?.startsWith('noctis:')) return null;

	const rest = description.slice('noctis:'.length);
...
function getUserDescription(schema: ZodTypeAny): string | undefined {
	...
	const noctis = parseNoctisTypeAnnotation(description);
	if (noctis) return noctis.userDescription;
	return description;
}
...
function getNoctisCustomTypeSchema(schema: ZodTypeAny, depth = 0): OpenAPISchemaOrRef | null {
	...
	const noctis = parseNoctisTypeAnnotation(description);
	if (noctis) {
		const ref = getRefForCustomTypeName(noctis.typeName);
		if (ref) return ref;

		if (noctis.typeName === 'Int32Enum') {
			if (noctis.bitflagTypeName) {
				const schemaName = noctis.bitflagTypeName;
				if (!int32EnumSchemaRegistry.has(schemaName)) {
					const schema: OpenAPISchema = {type: 'integer', format: 'int32'};
					if (noctis.enumEntries && noctis.enumEntries.length > 0) {
						const enumValues = noctis.enumEntries.map((entry) => entry.value);
						if (enumValues.every((value) => typeof value === 'number')) {
							schema.enum = enumValues as Array<number>;
							(schema as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctis.enumEntries.map(
								(e) => e.name,
							);
							const descriptions: Array<string | null> = noctis.enumEntries.map((e) => e.description ?? null);
							if (descriptions.some((d) => d != null)) {
								(schema as OpenAPISchema & Record<string, unknown>)['x-enumDescriptions'] = descriptions;
							}
						}
					}
					if (noctis.userDescription) {
						schema.description = noctis.userDescription;
					}
					int32EnumSchemaRegistry.set(schemaName, schema);
				}
				if (noctis.fieldDescription) {
					return {$ref: `#/components/schemas/${schemaName}`, description: noctis.fieldDescription};
				}
			...
			if (noctis.enumEntries && noctis.enumEntries.length > 0) {
				...
				if (enumValues.every((value) => typeof value === 'number')) {
					...
					(result as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctis.enumEntries.map((e) => e.name);
					const descriptions: Array<string | null> = noctis.enumEntries.map((e) => e.description ?? null);
					if (descriptions.some((d) => d != null)) {
		...
		if (noctis.typeName === 'Bitflags64') {
			if (noctis.bitflagTypeName) {
				const schemaName = noctis.bitflagTypeName;
				if (!bitflagSchemaRegistry.has(schemaName)) {
					const schema: OpenAPISchema = {type: 'string', format: 'int64', pattern: '^[0-9]+$'};
					if (noctis.bitflagValues && noctis.bitflagValues.length > 0) {
						(schema as OpenAPISchema & Record<string, unknown>)['x-bitflagValues'] = noctis.bitflagValues;
					}
					if (noctis.userDescription) {
						schema.description = noctis.userDescription;
					}
			...
			if (noctis.bitflagValues && noctis.bitflagValues.length > 0) {
				(result as OpenAPISchema & Record<string, unknown>)['x-bitflagValues'] = noctis.bitflagValues;
			}
			return result;
		}

		if (noctis.typeName === 'Bitflags32') {
			if (noctis.bitflagTypeName) {
				const schemaName = noctis.bitflagTypeName;
				if (!bitflagSchemaRegistry.has(schemaName)) {
		...
		if (noctis.typeName === 'Permissions') {
			if (noctis.bitflagTypeName) {
				const schemaName = noctis.bitflagTypeName;
				if (!bitflagSchemaRegistry.has(schemaName)) {
					const schema: OpenAPISchema = {type: 'string', format: 'int64', pattern: '^[0-9]+$'};
		...
		const customSchema = NOCTIS_CUSTOM_TYPES[noctis.typeName];
	...
	if (zodTypeName === 'ZodEffects' || zodTypeName === 'effect' || zodTypeName === 'pipe') {
		...
		if (inner) {
			...
			const innerCustomSchema = getNoctisCustomTypeSchema(inner, depth + 1);
...
function isSnowflakeType(schema: ZodTypeAny, depth = 0): boolean {
	...
	const customTypeSchema = getNoctisCustomTypeSchema(schema, depth);
...
export function zodToOpenAPISchema(schema: ZodTypeAny, depth = 0): OpenAPISchemaOrRef {
	...
	const customTypeSchema = getNoctisCustomTypeSchema(schema);
	if (customTypeSchema) {
	...
	switch (typeName) {
		...
		case 'number': {
			...
			const description = getDescription(schema);
			const noctis = parseNoctisTypeAnnotation(description);
			if (noctis?.typeName === 'EnumValues' && noctis.enumEntries && noctis.enumEntries.length > 0) {
				const enumValues = noctis.enumEntries.map((entry) => entry.value);
				if (enumValues.every((value) => typeof value === 'number')) {
					result.enum = enumValues as Array<number>;
					(result as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctis.enumEntries.map((e) => e.name);
		...
		case 'union': {
			...
			if (noctis?.typeName === 'FlexibleEnumValues' && noctis.enumEntries && noctis.enumEntries.length > 0) {
				const result: OpenAPISchema = {type: 'string'};
				(result as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctis.enumEntries.map((e) => e.name);
				const descriptions: Array<string | null> = noctis.enumEntries.map((e) => e.description ?? null);
				if (descriptions.some((d) => d != null)) {
				...
				const knownValues = noctis.enumEntries.map((e) => String(e.value)).join(', ');
				const baseDescription = noctis.userDescription ?? '';
			...
			if (allLiterals) {
				...
				if (literalValues.every((vals) => Array.isArray(vals) && vals.length > 0)) {
					...
					const noctisForLiterals = parseNoctisTypeAnnotation(description);
					if (
						noctisForLiterals?.typeName === 'EnumValues' &&
						noctisForLiterals.enumNames &&
						noctisForLiterals.enumNames.length === flattened.length
					) {
						(literalSchema as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctisForLiterals.enumNames;
						if (noctisForLiterals.enumEntries && noctisForLiterals.enumEntries.length === flattened.length) {
							const descriptions: Array<string | null> = [];
							for (let i = 0; i < flattened.length; i++) {
								const literalValue = flattened[i];
								const entry = noctisForLiterals.enumEntries[i];
		...
		case 'literal': {
			...
			if (values && values.length > 0) {
				...
				const description = getDescription(schema);
				const noctis = parseNoctisTypeAnnotation(description);
				if (noctis?.typeName === 'EnumValue' && noctis.enumNames && noctis.enumNames.length > 0) {
					(literalSchema as OpenAPISchema & Record<string, unknown>)['x-enumNames'] = noctis.enumNames;
					if (noctis.enumEntries && noctis.enumEntries.length > 0 && noctis.enumEntries[0].description) {
						(literalSchema as OpenAPISchema & Record<string, unknown>)['x-enumDescriptions'] = [
							noctis.enumEntries[0].description,
						];
					}
				}
				return addDescription(literalSchema, schema);
...
export const NOCTIS_CUSTOM_TYPES: Record<string, OpenAPISchema> = {
...
export function getCustomTypeSchema(typeName: string): OpenAPISchema | null {
	return NOCTIS_CUSTOM_TYPES[typeName] ?? null;
}
