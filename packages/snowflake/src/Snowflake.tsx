...
export const NOCTIS_EPOCH = BigInt(NOCTIS_EPOCH_NUMBER);
...
function toRelativeTimestamp(timestamp: number | bigint): bigint {
	...
	const relativeTimestamp = timestampBigInt - NOCTIS_EPOCH;

	if (relativeTimestamp < 0n) {
		throw new Error('Timestamp must be on or after the Noctis epoch');
	}
...
function toEpochTimestamp(relativeTimestamp: bigint): bigint {
	return relativeTimestamp + NOCTIS_EPOCH;
}
...
function getTimestampFromNow(now: () => number): bigint {
	return BigInt(now()) - NOCTIS_EPOCH;
}
...
export function isValidSnowflake(value: unknown): value is bigint {
	...
	if (timestampNumber < Number(NOCTIS_EPOCH)) {
		return false;
	}
