...
const NOCTIS_EPOCH_NUMBER = Number(NOCTIS_EPOCH);
...
function toClampedTimestamp(timestamp: number): number {
	if (timestamp <= NOCTIS_EPOCH_NUMBER) {
		return NOCTIS_EPOCH_NUMBER;
	}
...
export function extractTimestamp(snowflake: string): number {
	try {
		return extractTimestampWithEpoch(BigInt(snowflake), NOCTIS_EPOCH);
	} catch (_error) {
		return Number.NaN;
	}
}
...
export function extractTimestampFromSnowflake(snowflake: string, epoch?: string | bigint): number {
	try {
		const epochBigInt = epoch != null ? (typeof epoch === 'string' ? BigInt(epoch) : epoch) : NOCTIS_EPOCH;
