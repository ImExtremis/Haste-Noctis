...
import {getConfig, loadConfig, resetConfig} from '@noctis/config/src/ConfigLoader';
import {type ConfigObject, deepMerge} from '@noctis/config/src/config_loader/ConfigObjectMerge';
import {afterEach, beforeEach, describe, expect, test, vi} from 'vitest';

function createTempConfig(config: Record<string, unknown>): string {
	const dir = mkdtempSync(path.join(tmpdir(), 'noctis-config-test-'));
...
describe('ConfigLoader', () => {
	...
	test('throws when config paths array is empty', async () => {
		await expect(loadConfig([])).rejects.toThrow('NOCTIS_CONFIG must be set');
	});
