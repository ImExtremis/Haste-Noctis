...
import {buildEnvOverrides, parseEnvValue, setNestedValue} from '@noctis/config/src/config_loader/EnvironmentOverrides';
...
describe('buildEnvOverrides', () => {
	test('extracts env vars with the given prefix', () => {
		const env = {
			NOCTIS_CONFIG__ENV: 'production',
			NOCTIS_CONFIG__DATABASE__HOST: 'db.example.com',
			UNRELATED_VAR: 'ignored',
		} as NodeJS.ProcessEnv;

		const result = buildEnvOverrides(env, 'NOCTIS_CONFIG__');
	...
	test('lowercases key segments', () => {
		const env = {
			NOCTIS_CONFIG__DATABASE__PORT: '5432',
		} as NodeJS.ProcessEnv;
	...
	test('skips undefined values', () => {
		const env = {
			NOCTIS_CONFIG__MISSING: undefined,
		} as NodeJS.ProcessEnv;
