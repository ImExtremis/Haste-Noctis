...
} from '@noctis/config/src/EndpointDerivation';
...
describe('deriveDomain', () => {
	const baseConfig: DomainConfig = {
		base_domain: 'noctis.dev',
		public_scheme: 'https',
		internal_scheme: 'http',
	};

	test('uses base domain for api endpoint', () => {
		expect(deriveDomain('api', baseConfig)).toBe('noctis.dev');
	});

	test('uses base domain for app endpoint', () => {
		expect(deriveDomain('app', baseConfig)).toBe('noctis.dev');
	});

	test('uses base domain for gateway endpoint', () => {
		expect(deriveDomain('gateway', baseConfig)).toBe('noctis.dev');
	});

	test('uses base domain for media endpoint', () => {
		expect(deriveDomain('media', baseConfig)).toBe('noctis.dev');
	});

	test('uses custom static CDN domain when specified', () => {
		const config = {...baseConfig, static_cdn_domain: 'cdn.noctis.dev'};
		expect(deriveDomain('static_cdn', config)).toBe('cdn.noctis.dev');
	});

	test('uses base domain for static CDN when custom domain not specified', () => {
		expect(deriveDomain('static_cdn', baseConfig)).toBe('static.noctis.app');
	});

	test('uses custom invite domain when specified', () => {
		const config = {...baseConfig, invite_domain: 'noctis.gg'};
		expect(deriveDomain('invite', config)).toBe('noctis.gg');
	});

	test('uses base domain for invite when custom domain not specified', () => {
		expect(deriveDomain('invite', baseConfig)).toBe('noctis.dev');
	});

	test('uses custom gift domain when specified', () => {
		const config = {...baseConfig, gift_domain: 'noctis.gift'};
		expect(deriveDomain('gift', config)).toBe('noctis.gift');
	});

	test('uses base domain for gift when custom domain not specified', () => {
		expect(deriveDomain('gift', baseConfig)).toBe('noctis.dev');
	});
});
...
describe('deriveEndpointsFromDomain', () => {
	describe('development environment (localhost)', () => {
		...
		test('derives static CDN endpoint via CDN host', () => {
			expect(endpoints.static_cdn).toBe('https://static.noctis.app');
		});
	...
	describe('production environment (standard HTTPS port)', () => {
		...
		test('derives api endpoint without port', () => {
			expect(endpoints.api).toBe('https://noctis.app/api');
		});

		test('derives api client endpoint without port', () => {
			expect(endpoints.api_client).toBe('https://noctis.app/api');
		});

		test('derives app endpoint without port', () => {
			expect(endpoints.app).toBe('https://noctis.app');
		});

		test('derives gateway endpoint with wss scheme without port', () => {
			expect(endpoints.gateway).toBe('wss://noctis.app/gateway');
		});

		test('derives media endpoint without port', () => {
			expect(endpoints.media).toBe('https://noctis.app/media');
		});
		...
		test('derives admin endpoint without port', () => {
			expect(endpoints.admin).toBe('https://noctis.app/admin');
		});

		test('derives marketing endpoint without port', () => {
			expect(endpoints.marketing).toBe('https://noctis.app/marketing');
		});

		test('derives invite endpoint without port', () => {
			expect(endpoints.invite).toBe('https://noctis.app/invite');
		});

		test('derives gift endpoint without port', () => {
			expect(endpoints.gift).toBe('https://noctis.app/gift');
		});
	});

	describe('staging environment (custom port)', () => {
		const stagingConfig: DomainConfig = {
			base_domain: 'staging.noctis.dev',
			public_scheme: 'https',
			internal_scheme: 'http',
			public_port: 8443,
			internal_port: 8080,
		...
		test('derives api endpoint with custom port', () => {
			expect(endpoints.api).toBe('https://staging.noctis.dev:8443/api');
		});

		test('derives api client endpoint with custom port', () => {
			expect(endpoints.api_client).toBe('https://staging.noctis.dev:8443/api');
		});

		test('derives app endpoint with custom port', () => {
			expect(endpoints.app).toBe('https://staging.noctis.dev:8443');
		});

		test('derives gateway endpoint with wss and custom port', () => {
			expect(endpoints.gateway).toBe('wss://staging.noctis.dev:8443/gateway');
		});
	});
	...
	describe('custom CDN domain', () => {
		const staticCdnConfig: DomainConfig = {
			...
			internal_scheme: 'http',
			public_port: 443,
			static_cdn_domain: 'cdn.noctis.app',
		};

		const endpoints = deriveEndpointsFromDomain(staticCdnConfig);

		test('uses custom CDN domain', () => {
			expect(endpoints.static_cdn).toBe('https://cdn.noctis.app');
		});
	...
	describe('custom invite and gift domains', () => {
		const customConfig: DomainConfig = {
			...
			internal_scheme: 'http',
			public_port: 443,
			invite_domain: 'noctis.gg',
			gift_domain: 'noctis.gift',
		};

		const endpoints = deriveEndpointsFromDomain(customConfig);

		test('uses custom invite domain', () => {
			expect(endpoints.invite).toBe('https://noctis.gg/invite');
		});

		test('uses custom gift domain', () => {
			expect(endpoints.gift).toBe('https://noctis.gift/gift');
		});
	...
	describe('canary environment', () => {
		const canaryConfig: DomainConfig = {
			base_domain: 'canary.noctis.app',
			public_scheme: 'https',
			internal_scheme: 'http',
			public_port: 443,
			static_cdn_domain: 'cdn-canary.noctis.app',
		};

		const endpoints = deriveEndpointsFromDomain(canaryConfig);

		test('derives api endpoint for canary', () => {
			expect(endpoints.api).toBe('https://canary.noctis.app/api');
		});

		test('derives app endpoint for canary', () => {
			expect(endpoints.app).toBe('https://canary.noctis.app');
		});

		test('derives gateway endpoint for canary', () => {
			expect(endpoints.gateway).toBe('wss://canary.noctis.app/gateway');
		});

		test('uses custom CDN domain for canary', () => {
			expect(endpoints.static_cdn).toBe('https://cdn-canary.noctis.app');
		});
	});
