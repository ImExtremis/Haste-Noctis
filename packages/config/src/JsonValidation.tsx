...
import ConfigSchema from '@noctis/config/src/ConfigSchema.json';
import {type ConfigObject, isPlainObject} from '@noctis/config/src/config_loader/ConfigObjectMerge';
...
function warnOnUnknownConfigKeys(config: ConfigObject): void {
	...
	console.warn(
		[
			`Config JSON contains unknown properties; they are ignored by Noctis.${extra}`,
			...shown.map((w) => `- ${w}`),
		].join('\n'),
	);
}
