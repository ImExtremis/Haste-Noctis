...
export class TestEmailService implements ITestEmailService {
	...
	async sendNewLoginAlertEmail(
		...
		this.logger.info(
			`New login alert email sent to ${email} for user ${username}, IP: ${ipAddress}, location: ${location}`,
		);
		return this.record(email, 'new_login_alert', {ip: ipAddress, location, user_agent: userAgent});
	}
