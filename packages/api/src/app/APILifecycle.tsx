...
import {GuildDataRepository} from '@noctis/api/src/guild/repositories/GuildDataRepository';
...
import {initializeMetricsService} from '@noctis/api/src/infrastructure/MetricsService';
import {InstanceConfigRepository} from '@noctis/api/src/instance/InstanceConfigRepository';
import {ipBanCache} from '@noctis/api/src/middleware/IpBanMiddleware';
import {initializeServiceSingletons, shutdownReportService} from '@noctis/api/src/middleware/ServiceMiddleware';
import {
...
import {NatsApiRpcListener} from '@noctis/api/src/rpc/NatsApiRpcListener';
import {initializeSearch, shutdownSearch} from '@noctis/api/src/SearchFactory';
import {warmupAdminSearchIndexes} from '@noctis/api/src/search/SearchWarmup';
import {VisionarySlotInitializer} from '@noctis/api/src/stripe/VisionarySlotInitializer';
import {UserRepository} from '@noctis/api/src/user/repositories/UserRepository';
import {VoiceDataInitializer} from '@noctis/api/src/voice/VoiceDataInitializer';
...
import {NatsConnectionManager} from '@noctis/nats/src/NatsConnectionManager';
...
export function createInitializer(config: APIConfig, logger: ILogger): () => Promise<void> {
	return async (): Promise<void> => {
		...
		if (searchInitialized) {
			const warmupLockKey = 'noctis:search:warmup:admin';
