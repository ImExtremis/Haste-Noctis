...
import {AuditLogMiddleware} from '@noctis/api/src/middleware/AuditLogMiddleware';
import {ConcurrencyLimitMiddleware} from '@noctis/api/src/middleware/ConcurrencyLimitMiddleware';
import {GuildAvailabilityMiddleware} from '@noctis/api/src/middleware/GuildAvailabilityMiddleware';
import {IpBanMiddleware} from '@noctis/api/src/middleware/IpBanMiddleware';
import {LocaleMiddleware} from '@noctis/api/src/middleware/LocaleMiddleware';
import {MetricsMiddleware} from '@noctis/api/src/middleware/MetricsMiddleware';
import {RequestCacheMiddleware} from '@noctis/api/src/middleware/RequestCacheMiddleware';
import {RequireXForwardedForMiddleware} from '@noctis/api/src/middleware/RequireXForwardedForMiddleware';
import {ServiceMiddleware} from '@noctis/api/src/middleware/ServiceMiddleware';
import {UserMiddleware} from '@noctis/api/src/middleware/UserMiddleware';
import type {HonoApp, HonoEnv} from '@noctis/api/src/types/HonoEnv';
import {InvalidApiOriginError} from '@noctis/errors/src/domains/core/InvalidApiOriginError';
...
import {formatTraceparent, getActiveSpan} from '@noctis/telemetry/src/Tracing';
...
export function configureMiddleware(routes: HonoApp, options: MiddlewarePipelineOptions): void {
	...
	applyMiddlewareStack(routes, {
		...
		// Phase F: Security headers on every API response
		securityHeaders: {
			// HSTS enabled only in production (requires HTTPS termination by proxy)
			hsts: nodeEnv === 'production',
			hstsMaxAge: 63_072_000, // 2 years
			hstsIncludeSubDomains: true,
			// API never serves HTML; these are strict no-op policies
	...
	if (nodeEnv === 'production') {
		routes.use('*', async (ctx, next) => {
			...
			if (ctx.req.method !== 'GET' && (host === 'web.noctis.app' || host === 'web.canary.noctis.app')) {
