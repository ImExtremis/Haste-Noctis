...
describe('DownloadService.resolveLatestDesktopRedirect', () => {
	...
	it('falls back to x64 artefact when manifest points to arm64 file', async () => {
		...
		const manifestBody = buildManifest('noctis-stable-0.0.8-arm64.dmg');
		...
		const listObjects = vi
			...
			.mockResolvedValue([
				{key: 'desktop/stable/darwin/x64/noctis-stable-0.0.8-arm64.dmg'},
				{key: 'desktop/stable/darwin/x64/noctis-stable-0.0.8-x64.dmg'},
			]);
		...
		const result = await downloadService.resolveLatestDesktopRedirect({
			...
			requestUrl: 'https://api.noctis.app/dl/desktop/stable/darwin/x64/latest/dmg',
		});

		expect(result).toBe('https://api.noctis.app/dl/desktop/stable/darwin/x64/noctis-stable-0.0.8-x64.dmg');
		expect(listObjects).toHaveBeenCalledTimes(1);
	});

	it('uses manifest filename directly when architecture already matches', async () => {
		const manifestKey = 'desktop/stable/darwin/x64/manifest.json';
		const manifestBody = buildManifest('noctis-stable-0.0.8-x64.dmg');
