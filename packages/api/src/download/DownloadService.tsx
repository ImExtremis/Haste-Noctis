...
import type {
	...
	DesktopFormat,
	DesktopPlatform,
} from '@noctis/schema/src/domains/download/DownloadSchemas';
...
export class DownloadService {
	...
	private buildPossibleFilenames(
		...
		if (format === 'setup') {
			filenames.push(`noctis-${channel}-${version}-${archSuffix}-setup${ext}`);
			filenames.push(`Noctis-${channel}-${version}-${archSuffix}-Setup${ext}`);
			filenames.push(`noctis-${version}-${archSuffix}-setup${ext}`);
			filenames.push(`Noctis-${version}-${archSuffix}-Setup${ext}`);
		} else {
			filenames.push(`noctis-${channel}-${version}-${archSuffix}${ext}`);
			filenames.push(`noctis-${version}-${archSuffix}${ext}`);
			filenames.push(`Noctis-${channel}-${version}-${archSuffix}${ext}`);
			filenames.push(`Noctis-${version}-${archSuffix}${ext}`);
		}
