...
import {clearDonationTestEmails, listDonationTestEmails} from '@noctis/api/src/donation/tests/DonationTestUtils';
import {ProductType} from '@noctis/api/src/stripe/ProductRegistry';
import {createApiTestHarness} from '@noctis/api/src/test/ApiTestHarness';
import {
	createCheckoutCompletedEvent,
	createMockWebhookPayload,
	createStripeApiHandlers,
	type StripeWebhookEventData,
} from '@noctis/api/src/test/msw/handlers/StripeApiHandlers';
import {server} from '@noctis/api/src/test/msw/server';
...
describe('StripeWebhookService - checkout.session.completed', () => {
	...
	describe('premium checkout', () => {
		test('processes completed premium checkout session successfully', async () => {
			...
			const {PaymentRepository} = await import('@noctis/api/src/user/repositories/PaymentRepository');
			const {UserRepository} = await import('@noctis/api/src/user/repositories/UserRepository');
	...
	describe('donation checkout', () => {
		...
		test('records donation with valid email and subscription', async () => {
			...
			const {DonationRepository} = await import('@noctis/api/src/donation/DonationRepository');
