...
describe('Stripe Webhook Edge Cases', () => {
	...
	describe('Lifetime + subscription conflicts', () => {
		...
		test('cannot redeem stellar gift when already have visionary', async () => {
			const account = await createTestAccount(harness);
			...
			await createBuilder(harness, account.token)
				.post('/gifts/STELLARGIFT/redeem')
				.expect(400, APIErrorCodes.CANNOT_REDEEM_STELLAR_WITH_VISIONARY)
				.execute();
		});
