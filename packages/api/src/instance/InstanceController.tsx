...
import {getKeyManager} from '@noctis/api/src/federation/KeyManager';
...
import {API_CODE_VERSION} from '@noctis/constants/src/AppConstants';
import {FEDERATION_PROTOCOL_VERSION} from '@noctis/constants/src/Federation';
import {WellKnownNoctisResponse} from '@noctis/schema/src/domains/instance/InstanceSchemas';
...
export function InstanceController(app: Hono<HonoEnv>) {
	app.get(
		...
		OpenAPI({
			operationId: 'get_well_known_noctis',
			summary: 'Get instance discovery document',
			responseSchema: WellKnownNoctisResponse,
			statusCode: 200,
			security: [],
			tags: ['Instance'],
			description:
				'Returns the instance discovery document including API endpoints, feature flags, limits, and federation capabilities. This is the canonical discovery endpoint for all Noctis clients.',
		}),
		async (ctx) => {
			...
			if (Config.federation?.enabled) {
				...
				response.public_key = {
					id: `https://${instanceDomain}/.well-known/noctis#main-key`,
