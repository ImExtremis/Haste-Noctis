...
import type {CsamScanContext, ICsamScanQueueService} from '@noctis/api/src/csam/CsamScanQueueService';
import type {CsamResourceType, FrameSample, PhotoDnaMatchResult} from '@noctis/api/src/csam/CsamTypes';
...
import {recordCsamMatch, recordCsamScan, recordCsamScanDuration} from '@noctis/api/src/telemetry/CsamTelemetry';
...
export class SynchronousCsamScanner implements ISynchronousCsamScanner {
	...
	async scanMedia(params: ScanMediaParams): Promise<SynchronousCsamScanResult> {
		...
		return await withBusinessSpan(
			'noctis.csam.scan_media',
			'noctis.csam.scans.media',
	...
	async scanBase64(params: ScanBase64Params): Promise<SynchronousCsamScanResult> {
		...
		return await withBusinessSpan(
			'noctis.csam.scan_base64',
			'noctis.csam.scans.base64',
			{resource_type: context.resourceType},
			async () => {
				const startTime = Date.now();
