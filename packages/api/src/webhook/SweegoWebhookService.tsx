...
import {SuspiciousActivityFlags} from '@noctis/constants/src/UserConstants';
	...
	async handleWebhook(params: {
		...
		if (secret) {
			if (!webhookId || !timestamp || !signature) {
				getMetricsService().counter({name: 'noctis.sweego.webhooks.rejected', value: 1});
		...
		try {
			...
			getMetricsService().counter({name: 'noctis.sweego.webhooks.invalid_json', value: 1});
			Logger.error({parseError, body: body.slice(0, 1000)}, 'Failed to parse Sweego webhook JSON body');
			return {status: 400, body: 'Invalid JSON'};
		}

		await this.processEvent(event);
		getMetricsService().counter({name: 'noctis.sweego.webhooks.processed', value: 1});
