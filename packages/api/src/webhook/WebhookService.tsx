...
import type {ChannelID, GuildID, UserID, WebhookID, WebhookToken} from '@noctis/api/src/BrandedTypes';
import {createChannelID, createGuildID, createWebhookID, createWebhookToken} from '@noctis/api/src/BrandedTypes';
...
import {transform as GitHubTransform} from '@noctis/api/src/webhook/transformers/GitHubTransformer';
import {transform as SentryTransform} from '@noctis/api/src/webhook/transformers/SentryTransformer';
...
import {MAX_WEBHOOKS_PER_CHANNEL, MAX_WEBHOOKS_PER_GUILD} from '@noctis/constants/src/LimitConstants';
import {MaxWebhooksPerChannelError} from '@noctis/errors/src/domains/channel/MaxWebhooksPerChannelError';
import {UnknownChannelError} from '@noctis/errors/src/domains/channel/UnknownChannelError';
import {MaxWebhooksPerGuildError} from '@noctis/errors/src/domains/guild/MaxWebhooksPerGuildError';
import {UnknownWebhookError} from '@noctis/errors/src/domains/webhook/UnknownWebhookError';
import type {AllowedMentionsRequest} from '@noctis/schema/src/domains/message/SharedMessageSchemas';
import type {GitHubWebhook} from '@noctis/schema/src/domains/webhook/GitHubWebhookSchemas';
import type {SentryWebhook} from '@noctis/schema/src/domains/webhook/SentryWebhookSchemas';
import type {
	WebhookCreateRequest,
	WebhookMessageRequest,
	WebhookTokenUpdateRequest,
	WebhookUpdateRequest,
} from '@noctis/schema/src/domains/webhook/WebhookRequestSchemas';
...
export class WebhookService {
	...
	async createWebhook(
		...
		getMetricsService().counter({name: 'noctis.webhooks.created', value: 1});
		return webhook;
	}
	...
	async deleteWebhook(
		...
		getMetricsService().counter({name: 'noctis.webhooks.deleted', value: 1});
	}
	...
	async executeWebhook({
		...
		try {
			...
			getMetricsService().histogram({name: 'noctis.webhook.execution.latency', valueMs: Date.now() - start});
		}
	}
