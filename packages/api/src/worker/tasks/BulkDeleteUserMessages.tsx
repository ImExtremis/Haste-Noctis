...
import {MessageDeletionService} from '@noctis/api/src/worker/services/MessageDeletionService';
...
const bulkDeleteUserMessages: WorkerTaskHandler = async (payload, helpers) => {
	...
	return await withSpan(
		{
			name: 'noctis.worker.bulk_delete_user_messages',
		...
		async () => {
			try {
				...
				recordCounter({
					name: 'noctis.worker.messages.bulk_deleted',
					dimensions: {
						status: 'success',
						user_id: userIdStr,
					},
				...
				recordHistogram({
					name: 'noctis.worker.bulk_delete.duration',
					valueMs: duration,
					dimensions: {
						user_id: userIdStr,
					},
				...
				recordHistogram({
					name: 'noctis.worker.bulk_delete.count',
