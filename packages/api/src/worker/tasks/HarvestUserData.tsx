...
import type {AdminArchive} from '@noctis/api/src/admin/models/AdminArchiveModel';
import {type ChannelID, createUserID, type GuildID, type MessageID, type UserID} from '@noctis/api/src/BrandedTypes';
import {Config} from '@noctis/api/src/Config';
import {makeAttachmentCdnUrl} from '@noctis/api/src/channel/services/message/MessageHelpers';
...
import {resolveSessionClientInfo} from '@noctis/api/src/utils/UserAgentUtils';
import {
	appendAssetToArchive,
	buildHashedAssetKey,
	getAnimatedAssetExtension,
} from '@noctis/api/src/worker/utils/AssetArchiveHelpers';
...
async function createAndUploadArchive(params: ArchiveParams): Promise<ArchiveResult> {
	...
	const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'noctis-harvest-'));
