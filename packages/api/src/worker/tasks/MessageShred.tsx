...
import {chunkArray, createBulkDeleteDispatcher} from '@noctis/api/src/worker/tasks/utils/MessageDeletion';
import {getWorkerDependencies} from '@noctis/api/src/worker/WorkerContext';
import {recordCounter, recordHistogram} from '@noctis/telemetry/src/Metrics';
...
const messageShredTask: WorkerTaskHandler = async (payload, helpers) => {
	...
	return await withSpan(
		{
			name: 'noctis.worker.message_shred',
		...
		async () => {
			try {
				...
				recordCounter({
					name: 'noctis.worker.messages.shred_processed',
					dimensions: {
						status: 'success',
						job_id: data.job_id,
				...
				recordHistogram({
					name: 'noctis.worker.message_shred.duration',
					valueMs: duration,
					dimensions: {
						job_id: data.job_id,
						target_user_id: targetUserIdStr,
				...
				recordHistogram({
					name: 'noctis.worker.message_shred.processed_count',
				...
				recordHistogram({
					name: 'noctis.worker.message_shred.skipped_count',
