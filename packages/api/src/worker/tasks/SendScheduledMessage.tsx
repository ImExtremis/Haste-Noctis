...
} from '@noctis/api/src/worker/executors/ScheduledMessageExecutor';
...
import type {WorkerTaskHelpers} from '@noctis/worker/src/contracts/WorkerTask';
import {z} from 'zod';
...
export async function sendScheduledMessage(payload: unknown, helpers: WorkerTaskHelpers): Promise<void> {
	...
	try {
		const result = await withSpan(
			{
				name: 'noctis.message.scheduled.execute',
				attributes: {
		...
		recordCounter({
			name: 'noctis.messages.scheduled_executed',
			dimensions: {status: 'success'},
			value: 1,
		});

		recordHistogram({
			name: 'noctis.message.scheduled_execution.latency',
