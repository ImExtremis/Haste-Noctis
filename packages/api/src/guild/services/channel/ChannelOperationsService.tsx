...
import type {ChannelID, EmojiID, GuildID, RoleID, StickerID, UserID} from '@noctis/api/src/BrandedTypes';
import {createChannelID, createRoleID, createUserID} from '@noctis/api/src/BrandedTypes';
...
import type {PermissionOverwrite} from '@noctis/api/src/database/types/ChannelTypes';
import type {GuildAuditLogService} from '@noctis/api/src/guild/GuildAuditLogService';
import type {AuditLogChange, GuildAuditLogChange} from '@noctis/api/src/guild/GuildAuditLogTypes';
import type {IGuildRepositoryAggregate} from '@noctis/api/src/guild/repositories/IGuildRepositoryAggregate';
import {ChannelHelpers, type ChannelReorderOperation} from '@noctis/api/src/guild/services/channel/ChannelHelpers';
...
import {ChannelTypes, Permissions} from '@noctis/constants/src/ChannelConstants';
import {GuildFeatures} from '@noctis/constants/src/GuildConstants';
import {MAX_CHANNELS_PER_CATEGORY, MAX_GUILD_CHANNELS} from '@noctis/constants/src/LimitConstants';
...
import {ResourceLockedError} from '@noctis/errors/src/domains/core/ResourceLockedError';
import {MaxGuildChannelsError} from '@noctis/errors/src/domains/guild/MaxGuildChannelsError';
...
} from '@noctis/schema/src/domains/channel/GuildChannelOrdering';
...
export class ChannelOperationsService {
	...
	async setChannelPermissionOverwrite(params: {
		...
		if (changeSet.length > 0) {
			...
			getMetricsService().counter({
				name: 'noctis.channels.permissions_updated',
				dimensions: {
