...
import type {LookupUserRequest} from '@noctis/schema/src/domains/admin/AdminUserSchemas';
...
export class AdminUserLookupService {
	...
	async lookupUser(data: LookupUserRequest) {
		...
		const noctisTagMatch = query.match(/^(.+)#(\d{1,4})$/);
		if (noctisTagMatch) {
			const username = noctisTagMatch[1];
			const discriminator = parseInt(noctisTagMatch[2], 10);
