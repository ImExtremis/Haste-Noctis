...
import type {IAdminRepository} from '@noctis/api/src/admin/IAdminRepository';
import {SystemDmJobRepository} from '@noctis/api/src/admin/repositories/SystemDmJobRepository';
import {AdminAssetPurgeService} from '@noctis/api/src/admin/services/AdminAssetPurgeService';
import {AdminAuditService} from '@noctis/api/src/admin/services/AdminAuditService';
import {AdminCodeGenerationService} from '@noctis/api/src/admin/services/AdminCodeGenerationService';
import {AdminGuildService} from '@noctis/api/src/admin/services/AdminGuildService';
import {AdminMessageDeletionService} from '@noctis/api/src/admin/services/AdminMessageDeletionService';
import {AdminMessageService} from '@noctis/api/src/admin/services/AdminMessageService';
import {AdminMessageShredService} from '@noctis/api/src/admin/services/AdminMessageShredService';
import {AdminReportService} from '@noctis/api/src/admin/services/AdminReportService';
import {AdminSearchService} from '@noctis/api/src/admin/services/AdminSearchService';
import {AdminSnowflakeReservationService} from '@noctis/api/src/admin/services/AdminSnowflakeReservationService';
import {AdminUserService} from '@noctis/api/src/admin/services/AdminUserService';
import {AdminVisionarySlotService} from '@noctis/api/src/admin/services/AdminVisionarySlotService';
import {AdminVoiceService} from '@noctis/api/src/admin/services/AdminVoiceService';
import {SystemDmService} from '@noctis/api/src/admin/services/SystemDmService';
import type {AuthService} from '@noctis/api/src/auth/AuthService';
import type {AttachmentID, ChannelID, GuildID, ReportID, UserID} from '@noctis/api/src/BrandedTypes';
...
import type {KVBulkMessageDeletionQueueService} from '@noctis/api/src/infrastructure/KVBulkMessageDeletionQueueService';
...
import {VisionarySlotRepository} from '@noctis/api/src/user/repositories/VisionarySlotRepository';
import type {UserContactChangeLogService} from '@noctis/api/src/user/services/UserContactChangeLogService';
import type {VoiceRepository} from '@noctis/api/src/voice/VoiceRepository';
...
} from '@noctis/schema/src/domains/admin/AdminGuildSchemas';
import type {
	...
	MessageShredRequest,
} from '@noctis/schema/src/domains/admin/AdminMessageSchemas';
import type {
...
} from '@noctis/schema/src/domains/admin/AdminUserSchemas';
import type {
...
} from '@noctis/schema/src/domains/admin/AdminVoiceSchemas';
...
export class AdminService {
	...
	async banIp(data: {ip: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.ip.created',
			dimensions: {
	...
	async unbanIp(data: {ip: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.ip.deleted',
			dimensions: {
				reason_type: auditLogReason ? 'provided' : 'none',
			},
		});
	...
	async banEmail(data: {email: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.email.created',
			dimensions: {
				reason_type: auditLogReason ? 'provided' : 'none',
			},
		});
	...
	async unbanEmail(data: {email: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.email.deleted',
			dimensions: {
				reason_type: auditLogReason ? 'provided' : 'none',
			},
		});
	...
	async banPhone(data: {phone: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.phone.created',
			dimensions: {
				reason_type: auditLogReason ? 'provided' : 'none',
			},
		});
	...
	async unbanPhone(data: {phone: string}, adminUserId: UserID, auditLogReason: string | null) {
		...
		getMetricsService().counter({
			name: 'noctis.admin_bans.phone.deleted',
			dimensions: {
				reason_type: auditLogReason ? 'provided' : 'none',
			},
		});
