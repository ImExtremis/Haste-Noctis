...
import {MAX_BIO_LENGTH} from '@noctis/constants/src/LimitConstants';
...
import type {UserUpdateRequest} from '@noctis/schema/src/domains/user/UserRequestSchemas';
...
export class UserAccountProfileService {
	...
	private async processBioUpdate(params: {user: User; bio: string | null; updates: UserFieldUpdates}): Promise<void> {
		...
		if (bio !== user.bio) {
			getMetricsService().counter({name: 'noctis.users.bio_updated'});
	...
	private async processPronounsUpdate(params: {
		...
		if (pronouns !== user.pronouns) {
			getMetricsService().counter({name: 'noctis.users.pronouns_updated'});
	...
	private async processAccentColorUpdate(params: {
		...
		if (accentColor !== user.accentColor) {
			getMetricsService().counter({name: 'noctis.users.accent_color_updated'});
	...
	private async processAvatarUpdate(params: {
		...
		return await withBusinessSpan(
			'noctis.user.avatar_update',
			'noctis.users.avatars_updated',
			{
	...
	private async processBannerUpdate(params: {
		...
		getMetricsService().counter({name: 'noctis.users.banner_updated'});
