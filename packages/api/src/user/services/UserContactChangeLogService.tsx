...
import type {UserContactChangeLogRepository} from '@noctis/api/src/user/repositories/UserContactChangeLogRepository';
...
export class UserContactChangeLogService {
	...
	async recordDiff(params: RecordDiffParams): Promise<void> {
		...
		const oldTag = oldUser ? this.buildNoctisTag(oldUser) : null;
		const newTag = this.buildNoctisTag(newUser);
		if (oldTag !== newTag) {
			tasks.push(
				this.repo.insertLog({
					userId: newUser.id,
	...
	private buildNoctisTag(user: User | null): string | null {
		if (!user) return null;
