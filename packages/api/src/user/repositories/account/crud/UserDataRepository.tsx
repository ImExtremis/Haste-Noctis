...
import {EMPTY_USER_ROW, USER_COLUMNS} from '@noctis/api/src/database/types/UserTypes';
import {User} from '@noctis/api/src/models/User';
import {Users} from '@noctis/api/src/Tables';

const NOCTIS_BOT_USER_ID = 0n;
...
export class UserDataRepository {
	async findUnique(userId: UserID): Promise<User | null> {
		if (userId === NOCTIS_BOT_USER_ID) {
			return new User({
				...EMPTY_USER_ROW,
				user_id: createUserID(NOCTIS_BOT_USER_ID),
				username: 'Noctis',
				discriminator: 0,
				bot: true,
				system: true,
