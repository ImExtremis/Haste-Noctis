...
import {DefaultUserOnly, LoginRequiredAllowSuspicious} from '@noctis/api/src/middleware/AuthMiddleware';
import {CaptchaMiddleware} from '@noctis/api/src/middleware/CaptchaMiddleware';
import {LocalAuthMiddleware} from '@noctis/api/src/middleware/LocalAuthMiddleware';
...
import {SudoModeMiddleware} from '@noctis/api/src/middleware/SudoModeMiddleware';
...
export function AuthController(app: HonoApp) {
	...
	app.delete(
		...
		OpenAPI({
			...
			security: ['bearerToken', 'sessionToken'],
			tags: ['Auth'],
			description: 'Revoke a specific authentication session for the current user.',
		}),
		async (ctx) => {
			const user = ctx.get('user');
			const {sessionId} = ctx.req.valid('param');
			await ctx.get('authRequestService').logoutAuthSessions({user, data: {session_id_hashes: [sessionId]}});
			return ctx.body(null, 204);
		},
	);
	...
	app.delete(
		...
		OpenAPI({
			...
			security: ['bearerToken', 'sessionToken'],
			tags: ['Auth'],
			description: 'Revoke all authentication sessions except the current one.',
		}),
		async (ctx) => {
			const userId = ctx.get('user').id;
			const currentToken = ctx.get('authToken') ?? '';
			await ctx.get('authRequestService').terminateOtherSessions(userId, currentToken);
			return ctx.body(null, 204);
		},
	);
	...
	app.delete(
		'/auth/account',
		RateLimitMiddleware(RateLimitConfigs.AUTH_SESSIONS_LOGOUT), // Basic limit
		LoginRequiredAllowSuspicious,
		DefaultUserOnly,
		SudoModeMiddleware,
		...
		OpenAPI({
			...
			security: ['bearerToken', 'sessionToken'],
			tags: ['Auth'],
			description: 'Schedule the account for deletion. Deletion takes effect after 7 days.',
		}),
		async (ctx) => {
			const user = ctx.get('user');
			const body = ctx.req.valid('json');
			await requireSudoMode(ctx, user, body, ctx.get('authService'), ctx.get('authMfaService'));
			await ctx.get('authRequestService').scheduleAccountDeletion(user);
			return ctx.body(null, 204);
		},
	);
	...
	app.get(
		...
		async (ctx) => {
			const user = ctx.get('user');
			await ctx.get('authRequestService').cancelAccountDeletion(user);
			return ctx.body(null, 204);
		},
	);
