...
import {createEmailVerificationToken, createInviteCode, createUserID, type UserID} from '@noctis/api/src/BrandedTypes';
import {Config} from '@noctis/api/src/Config';
import {FIRST_ADMIN_ACL_CONFIG_KEY} from '@noctis/api/src/constants/InstanceConfig';
import {deleteOneOrMany, executeConditional} from '@noctis/api/src/database/Cassandra';
...
import {InstanceConfiguration, UserByEmail} from '@noctis/api/src/Tables';
...
import * as AgeUtils from '@noctis/api/src/utils/AgeUtils';
import * as FetchUtils from '@noctis/api/src/utils/FetchUtils';
import {getIpAddressReverse, lookupGeoip} from '@noctis/api/src/utils/IpUtils';
import {generateRandomUsername} from '@noctis/api/src/utils/UsernameGenerator';
import {deriveUsernameFromDisplayName} from '@noctis/api/src/utils/UsernameSuggestionUtils';
...
import {formatGeoipLocation, type GeoipResult, UNKNOWN_LOCATION} from '@noctis/geoip/src/GeoipLookup';
...
import type {RegisterRequest} from '@noctis/schema/src/domains/auth/AuthSchemas';
...
export class AuthRegistrationService {
	...
	async register({data, request, requestCache}: RegisterParams): Promise<{user_id: string; token: string}> {
		return await withBusinessSpan('noctis.auth.register', 'noctis.auth.registrations', {}, () =>
	...
	private async buildRegistrationMetadataContext(params: {
		...
		const noctisTag = `${user.username}#${user.discriminator.toString().padStart(4, '0')}`;
	...
	private async sendRegistrationWebhook(
		...
		const embedFields = [
			...
			{name: 'NoctisTag', value: context.noctisTag, inline: true},
