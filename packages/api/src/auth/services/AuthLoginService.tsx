...
import {UserAuthenticatorTypes, UserFlags} from '@noctis/constants/src/UserConstants';
...
import {IpAuthorizationRequiredError} from '@noctis/errors/src/domains/auth/IpAuthorizationRequiredError';
import {IpAuthorizationResendCooldownError} from '@noctis/errors/src/domains/auth/IpAuthorizationResendCooldownError';
import {IpAuthorizationResendLimitExceededError} from '@noctis/errors/src/domains/auth/IpAuthorizationResendLimitExceededError';
...
import {formatGeoipLocation, UNKNOWN_LOCATION} from '@noctis/geoip/src/GeoipLookup';
import {requireClientIp} from '@noctis/ip_utils/src/ClientIp';
import type {IRateLimitService, RateLimitResult} from '@noctis/rate_limit/src/IRateLimitService';
import type {LoginRequest} from '@noctis/schema/src/domains/auth/AuthSchemas';
...
export class AuthLoginService {
	...
	async completeIpAuthorization(token: string): Promise<{token: string; user_id: string; ticket: string}> {
		...
		if (payload.platform) {
			headers['x-noctis-platform'] = payload.platform;
		}

		const syntheticRequest = new Request('https://api.noctis.app/auth/ip-authorization', {
			headers,
			method: 'POST',
		});

		await this.checkAndRecordDevice(user, syntheticRequest);
	...
	> {
		return await withBusinessSpan('noctis.auth.login', 'noctis.auth.logins', {}, () =>
	...
	> {
		...
		if (!hasMfa && !isAppStoreReviewer) {
			...
			if (!isIpAuthorized) {
				...
				const platform = request.headers.get('x-noctis-platform');
		...
		await this.checkAndRecordDevice(currentUser, request);
	...
	async loginMfaTotp({code, ticket, request}: LoginMfaTotpParams): Promise<{user_id: string; token: string}> {
		...
		await this.checkAndRecordDevice(user, request);
	...
	private async checkAndRecordDevice(user: User, request: Request) {
		const clientIp = requireClientIp(request, {
			trustCfConnectingIp: Config.proxy.trust_cf_connecting_ip,
		});
		const userAgent = request.headers.get('user-agent') || '';
		
		const deviceString = `${clientIp}|${userAgent}`;
		const deviceHash = crypto.createHash('sha256').update(deviceString).digest('hex');

		const isNewDevice = await this.repository.checkAndRecordDevice(user.id, deviceHash);
		
		if (isNewDevice) {
			const geoipResult = await lookupGeoip(clientIp);
			const clientLocation = formatGeoipLocation(geoipResult) ?? UNKNOWN_LOCATION;

			this.emailService.sendNewLoginAlertEmail(
				user.email!,
				user.username,
				clientIp,
				userAgent,
				clientLocation,
				user.locale
			).catch(err => Logger.warn({userId: user.id, err}, 'Failed to send new login alert email'));
		}
	}
}
