...
import {signInitiationToken, verifyInitiationToken} from '@noctis/api/src/connection/ConnectionInitiationToken';
import {mapConnectionToResponse} from '@noctis/api/src/connection/ConnectionMappers';
import {ConnectionInitiationTokenInvalidError} from '@noctis/api/src/connection/errors/ConnectionInitiationTokenInvalidError';
import type {IConnectionService} from '@noctis/api/src/connection/IConnectionService';
import {
...
} from '@noctis/schema/src/domains/connection/ConnectionSchemas';
	...
	private generateVerificationInstructions(connectionType: ConnectionType, identifier: string): string {
		switch (connectionType) {
			case ConnectionTypes.DOMAIN:
				return `Add a DNS TXT record at _noctis.${identifier} with the value noctis-verification=<token>, or serve the token at https://${identifier}/.well-known/noctis-verification`;
			default:
