...
import {type ChannelID, createEmojiID, type MessageID, type UserID} from '@noctis/api/src/BrandedTypes';
...
import {
	...
	type ParsedEmoji,
} from '@noctis/api/src/channel/services/interaction/MessageInteractionBase';
import {isGuildMemberTimedOut} from '@noctis/api/src/guild/GuildModel';
...
import type {MessageReaction} from '@noctis/api/src/models/MessageReaction';
import type {User} from '@noctis/api/src/models/User';
import type {IUserRepository} from '@noctis/api/src/user/IUserRepository';
import {mapUserToPartialResponse} from '@noctis/api/src/user/UserMappers';
...
import {MAX_REACTIONS_PER_MESSAGE, MAX_USERS_PER_MESSAGE_REACTION} from '@noctis/constants/src/LimitConstants';
import {ValidationErrorCodes} from '@noctis/constants/src/ValidationErrorCodes';
import {MaxReactionsPerMessageError} from '@noctis/errors/src/domains/channel/MaxReactionsPerMessageError';
import {MaxUsersPerMessageReactionError} from '@noctis/errors/src/domains/channel/MaxUsersPerMessageReactionError';
...
import {CommunicationDisabledError} from '@noctis/errors/src/domains/moderation/CommunicationDisabledError';
...
export class MessageReactionService extends MessageInteractionBase {
	...
	async getUsersForReaction({
		...
		getMetricsService().counter({name: 'noctis.reactions.users_fetched'});
	...
	async removeAllReactionsForEmoji({
		...
		getMetricsService().counter({name: 'noctis.reactions.emoji_cleared'});
	}
	...
	async removeAllReactions({
		...
		getMetricsService().counter({name: 'noctis.reactions.all_cleared'});
	}
