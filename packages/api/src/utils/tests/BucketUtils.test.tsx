...
import {NOCTIS_EPOCH} from '@noctis/constants/src/Core';
import {makeBucket, makeBuckets} from '@noctis/snowflake/src/SnowflakeBuckets';
import {afterEach, beforeEach, describe, expect, it, vi} from 'vitest';

const TEN_DAYS_MS = 10 * 24 * 60 * 60 * 1000;

function createSnowflake(timestamp: number): bigint {
	const relativeTimestamp = timestamp - NOCTIS_EPOCH;
...
describe('makeBuckets', () => {
	...
	it('returns range of buckets from start to end', () => {
		const start = createSnowflake(NOCTIS_EPOCH);
		const end = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS * 2);
		expect(makeBuckets(start, end)).toEqual([0, 1, 2]);
	});

	it('returns buckets from start to current when end is null', () => {
		const now = NOCTIS_EPOCH + TEN_DAYS_MS * 3 + 1000;
		vi.setSystemTime(now);
		const start = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS);
		expect(makeBuckets(start, null)).toEqual([1, 2, 3]);
	});

	it('returns buckets from current to end when start is null', () => {
		const now = NOCTIS_EPOCH + TEN_DAYS_MS + 1000;
		vi.setSystemTime(now);
		const end = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS * 3);
	...
	it('returns many buckets for large time range', () => {
		const start = createSnowflake(NOCTIS_EPOCH);
		const end = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS * 9);
	...
	it('handles snowflakes within same bucket', () => {
		const start = createSnowflake(NOCTIS_EPOCH + 1000);
		const end = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS - 1000);
		expect(makeBuckets(start, end)).toEqual([0]);
	});

	it('returns ordered buckets from low to high', () => {
		const start = createSnowflake(NOCTIS_EPOCH);
		const end = createSnowflake(NOCTIS_EPOCH + TEN_DAYS_MS * 5);
		const buckets = makeBuckets(start, end);
