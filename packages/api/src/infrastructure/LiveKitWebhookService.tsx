...
import {
	...
	parseRoomName,
} from '@noctis/api/src/infrastructure/VoiceRoomContext';
...
import type {VoiceTopology} from '@noctis/api/src/voice/VoiceTopology';
...
export class LiveKitWebhookService {
		...
		try {
			...
			if (data.event.numDropped != null && data.event.numDropped > 0) {
				...
				getMetricsService().counter({
					name: 'noctis.voice.webhook.events_dropped',
			...
			getMetricsService().counter({
				name: 'noctis.livekit.webhooks.processed',
				value: 1,
				dimensions: {
			...
			getMetricsService().counter({
				name: 'noctis.livekit.webhooks.failed',
				value: 1,
				dimensions: {
	...
	async handleParticipantJoined(event: WebhookEvent): Promise<void> {
		...
		try {
			...
			if (!result.success) {
				...
				getMetricsService().counter({
					name: 'noctis.voice.webhook.join_rejected',
					value: 1,
			...
			getMetricsService().counter({
				name: 'noctis.voice.webhook.join_confirmed',
				value: 1,
			});
		} catch (error) {
	...
	private async isParticipantStillInRoom(params: {
		...
		if (result.status === 'error') {
			...
			getMetricsService().counter({
				name: 'noctis.voice.reconcile.lookup_error',
				value: 1,
			});
			return 'unknown';
		}
		...
		try {
			if (raw.region_id && raw.server_id) {
				...
				if (pinnedServer && (pinnedServer.regionId !== raw.region_id || pinnedServer.serverId !== raw.server_id)) {
					...
					getMetricsService().counter({
						name: 'noctis.voice.webhook.stale_server_event_ignored',
						value: 1,
					});
					return;
				}
			...
			if (presenceStatus === 'unknown') {
				...
				getMetricsService().counter({
					name: 'noctis.voice.reconcile.disconnect_ignored',
					value: 1,
				});
				return;
			}
