...
import type {IMetricsService} from '@noctis/api/src/infrastructure/IMetricsService';
import {parseParticipantIdentity, parseRoomName} from '@noctis/api/src/infrastructure/VoiceRoomContext';
import type {VoiceRoomStore} from '@noctis/api/src/infrastructure/VoiceRoomStore';
...
export class VoiceReconciliationWorker {
	...
	async reconcile(): Promise<void> {
		...
		this.metricsService.gauge({name: 'noctis.voice.reconcile.rooms_checked', value: roomsChecked});
		this.metricsService.counter({name: 'noctis.voice.reconcile.livekit_only_confirmed', value: totalConfirmed});
		this.metricsService.counter({name: 'noctis.voice.reconcile.livekit_only_disconnected', value: totalDisconnected});
		this.metricsService.counter({name: 'noctis.voice.reconcile.gateway_only_removed', value: totalGatewayRemoved});
		this.metricsService.gauge({name: 'noctis.voice.reconcile.consistent', value: totalConsistent});
		this.metricsService.counter({name: 'noctis.voice.reconcile.errors', value: totalErrors});
		this.metricsService.histogram({name: 'noctis.voice.reconcile.sweep_duration', valueMs: durationMs});
