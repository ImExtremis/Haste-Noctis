 ...
 * These are defined in @noctis/constants/src/GatewayConstants.tsx
 */
...
async function main() {
	...
	const gatewayDir = path.join(repoRoot, 'noctis_docs/gateway');
	const schemasDir = path.join(repoRoot, 'noctis_docs/schemas/events');
	const openapiPath = path.join(repoRoot, 'noctis_docs/api-reference/openapi.json');
	...
	closeCodesContent += createFrontmatter({
		title: 'Close codes',
		description: 'WebSocket close codes used by the Noctis gateway.',
	});
	closeCodesContent += '\n\n';
	closeCodesContent +=
	...
	eventsContent += createFrontmatter({
		title: 'Events',
		description: 'Gateway dispatch events sent by the Noctis gateway.',
	});
	eventsContent += '\n\n';
	eventsContent +=
