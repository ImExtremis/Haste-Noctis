...
function renderOverviewMdx(schemaGroups, allSchemaNames, state) {
	...
	out += `This section contains all resource schemas (data types) used by the Noctis API.\n\n`;
...
async function main() {
	...
	const outDir = path.join(repoRoot, 'noctis_docs/resources');
	...
	for (const [fileName, schemaNames] of schemaGroups) {
		...
		const mdx = renderMdxForSchemas(schemaNames, schemas, state, {
			title: displayName,
			description: `${displayName} object schemas from the Noctis API.`,
		});
