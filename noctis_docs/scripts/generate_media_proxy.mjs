...
const MEDIA_PROXY_BASE_URL = 'https://noctisusercontent.com';
...
function renderEndpointsMdx() {
	...
	out += createFrontmatter({
		title: 'Media proxy',
		description: 'Object schemas used by the Noctis media proxy.',
	});
	out += '\n\n';

	out += `The media proxy serves all user-generated content for Noctis, including avatars, icons, attachments, emojis, stickers, and external media. All public routes are served from \`${MEDIA_PROXY_BASE_URL}\`.\n\n`;
...
function generateOpenApiSpec() {
	const spec = {
		openapi: '3.1.0',
		info: {
			title: 'Noctis Media Proxy API',
			version: '1.0.0',
			description:
				'Media proxy API for Noctis. Serves avatars, icons, banners, attachments, emojis, stickers, and proxied external media.',
			contact: {
	...
	spec.paths['/external/{signature}/{timestamp}/{url}'] = {
		get: {
			...
			description:
				'Proxy external media through the Noctis media proxy. External URLs are signed with HMAC-SHA256 to prevent abuse. The signature and timestamp parameters ensure URLs cannot be forged and expire after a configured period.',
			tags: ['External'],
			parameters: [
				{
					name: 'signature',
...
async function main() {
	...
	await writeFile(path.join(repoRoot, 'noctis_docs/resources/media_proxy.mdx'), resourcesMdx);

	const openApiSpec = generateOpenApiSpec();
	await writeFile(
		path.join(repoRoot, 'noctis_docs/media-proxy-api/openapi.json'),
		JSON.stringify(openApiSpec, null, '\t'),
	);

	console.log('Generated media proxy documentation');
	console.log('  - noctis_docs/resources/media_proxy.mdx');
	console.log('  - noctis_docs/media-proxy-api/openapi.json');
}
