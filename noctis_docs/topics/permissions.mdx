...
Permissions in Noctis are stored as a bitfield, represented as a string containing the permission integer. Each permission is a bit position that can be set or unset. To check if a user has a permission, perform a bitwise AND operation between the user's permission value and the permission flag.
