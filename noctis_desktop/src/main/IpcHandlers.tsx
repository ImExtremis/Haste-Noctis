...
async function assertValidNoctisInstance(instanceOrigin: string): Promise<void> {
	const url = new URL('/.well-known/noctis', instanceOrigin).toString();
	...
	try {
		...
		throw new Error(`Not a valid Noctis instance (${message})`);
	} finally {
...
export function registerIpcHandlers(): void {
	ipcMain.handle('switch-instance-url', async (_event, options: SwitchInstanceUrlOptions): Promise<void> => {
		...
		await assertValidNoctisInstance(instanceOrigin);
