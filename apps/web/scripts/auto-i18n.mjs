...
const envOverrides = loadEnvFromFiles(['NOCTIS_AUTO_I18N', 'OPENROUTER_API_KEY']);
const NOCTIS_AUTO_I18N = process.env.NOCTIS_AUTO_I18N ?? envOverrides.NOCTIS_AUTO_I18N ?? '';
const OPENROUTER_API_KEY = process.env.OPENROUTER_API_KEY ?? envOverrides.OPENROUTER_API_KEY ?? '';

const shouldRun = NOCTIS_AUTO_I18N === '1' && Boolean(OPENROUTER_API_KEY);
if (!shouldRun) {
	process.exit(0);
}

const childEnv = {...process.env, NOCTIS_AUTO_I18N, OPENROUTER_API_KEY};
