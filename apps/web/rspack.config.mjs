...
const CDN_ENDPOINT = 'https://static.noctis.app';
...
function readConfig() {
	const configPath = process.env.NOCTIS_CONFIG;
	if (!configPath) {
		throw new Error('NOCTIS_CONFIG must be set to a JSON config path.');
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				'@app': SRC_DIR,
				'@pkgs': PKGS_DIR,
				'@noctis/constants/src': path.join(MONOREPO_ROOT, 'packages/constants/src'),
				'@noctis/date_utils/src': path.join(MONOREPO_ROOT, 'packages/date_utils/src'),
				'@noctis/geo_utils/src': path.join(MONOREPO_ROOT, 'packages/geo_utils/src'),
				'@noctis/limits/src': path.join(MONOREPO_ROOT, 'packages/limits/src'),
				'@noctis/list_utils/src': path.join(MONOREPO_ROOT, 'packages/list_utils/src'),
				'@noctis/markdown_parser/src': path.join(MONOREPO_ROOT, 'packages/markdown_parser/src'),
				'@noctis/number_utils/src': path.join(MONOREPO_ROOT, 'packages/number_utils/src'),
				'@noctis/schema/src': path.join(MONOREPO_ROOT, 'packages/schema/src'),
				'@noctis/snowflake/src': path.join(MONOREPO_ROOT, 'packages/snowflake/src'),
				'@noctis/ui/src': path.join(MONOREPO_ROOT, 'packages/ui/src'),
			},
			extensions: [
				'.web.tsx',
				'.web.ts',
		...
		devServer: {
			port: Number(process.env.NOCTIS_APP_DEV_PORT) || 49427,
			hot: false,
			liveReload: false,
			client: false,
