...
export const ReportPage = observer(() => {
	...
	useNoctisDocumentTitle(t`Report Illegal Content`);
		...
		(
			...
			if (error && typeof error === 'object' && 'body' in error && (error as {body?: unknown}).body) {
				...
				const pathMap: Record<string, keyof FormValues> = {
					...
					reporter_noctis_tag: 'reporterNoctisTag',
	...
	const handleSubmit = useCallback(async () => {
		...
		const reporterNoctisTag = state.formValues.reporterNoctisTag.trim();
		...
		if (reporterNoctisTag) payload.reporter_noctis_tag = reporterNoctisTag;
		...
		switch (state.selectedType) {
			...
			case 'user': {
				...
				if (!userId && !userTag) {
					dispatch({
						type: 'SET_ERROR',
						message: t`Provide either a user ID or a NoctisTag for the person you are reporting.`,
					});
					return;
				}
