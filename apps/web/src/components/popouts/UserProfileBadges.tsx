...
import {PublicUserFlags, UserPremiumTypes} from '@noctis/constants/src/UserConstants';
		...
		const badges = useMemo(() => {
			...
			if (!selfHosted && user.flags & PublicUserFlags.CTP_MEMBER) {
				result.push({
					...
					tooltip: t`Noctis Community Team`,
					url: Routes.careers(),
				});
			}
			...
			if (!selfHosted && user.flags & PublicUserFlags.BUG_HUNTER) {
				result.push({
					...
					tooltip: t`Noctis Bug Hunter`,
					url: Routes.bugs(),
				});
			}

			if (!selfHosted && profile?.premiumType && profile.premiumType !== UserPremiumTypes.NONE) {
				let tooltipText = t`Noctis Stellar`;
				let badgeUrl = Routes.stellar();
				...
				if (profile.premiumType === UserPremiumTypes.LIFETIME) {
					if (profile.premiumSince) {
						...
						tooltipText = `Noctis Visionary since ${premiumSinceFormatted}`;
					} else {
						tooltipText = `Noctis Visionary`;
					}
					badgeUrl = Routes.helpArticle('visionary');
				} else if (profile.premiumSince) {
					const premiumSinceFormatted = DateUtils.getFormattedShortDate(profile.premiumSince);
					tooltipText = `Noctis Stellar subscriber since ${premiumSinceFormatted}`;
				}

				result.push({
					type: 'icon',
					key: 'premium',
					iconUrl: cdnUrl('badges/stellar.svg'),
					tooltip: tooltipText,
					url: badgeUrl,
				});
