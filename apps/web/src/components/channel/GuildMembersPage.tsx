...
import {TimestampStyle} from '@noctis/markdown_parser/src/types/Enums';
import {extractTimestampFromSnowflakeAsDate} from '@noctis/snowflake/src/SnowflakeUtils';
...
const MembersTableView: React.FC<{guildId: string}> = observer(({guildId}) => {
	...
	const [joinedNoctisFilter, setJoinedNoctisFilter] = useState<DateRangeFilter>({});
	...
	const performSearch = useCallback(
		async (params: {query?: string; offsetVal?: number; append?: boolean}) => {
			...
			try {
				...
				if (joinedNoctisFilter.gte != null) {
					searchParams.user_created_at_gte = joinedNoctisFilter.gte;
				}
				if (joinedNoctisFilter.lte != null) {
					searchParams.user_created_at_lte = joinedNoctisFilter.lte;
				}
		...
		[guildId, sortMode, pageSize, roleFilter, memberSinceFilter, joinedNoctisFilter, joinMethodFilter],
	);
	...
	const handleJoinedNoctisFilterOpen = useCallback(
		(event: React.MouseEvent<HTMLButtonElement>) => {
			openDateRangeFilter(event, joinedNoctisFilter, setJoinedNoctisFilter);
		},
		[openDateRangeFilter, joinedNoctisFilter],
	);
	...
	const joinedNoctisActive = joinedNoctisFilter.gte != null || joinedNoctisFilter.lte != null;
							...
							<table className={styles.table}>
								<thead className={styles.tableHead}>
									<tr className={styles.tableHeadRow}>
										...
										<th className={clsx(styles.th, styles.dateCol)}>
											<div className={styles.thContent}>
												<Trans>Joined Noctis</Trans>
												<button
													type="button"
													className={clsx(styles.filterButton, joinedNoctisActive && styles.filterButtonActive)}
													onClick={handleJoinedNoctisFilterOpen}
												>
													<FunnelIcon size={12} weight={joinedNoctisActive ? 'fill' : 'bold'} />
												</button>
											</div>
										</th>
...
export const GuildMembersPage: React.FC<GuildMembersPageProps> = observer(({guildId}) => {
	...
	useNoctisDocumentTitle(useMemo(() => [t`Members`, guild?.name], [t, guild?.name]));
