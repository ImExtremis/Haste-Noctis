...
import {NOCTISBOT_ID} from '@noctis/constants/src/AppConstants';
import {MessageEmbedTypes, MessageFlags, MessageStates, MessageTypes} from '@noctis/constants/src/ChannelConstants';
import {NodeType} from '@noctis/markdown_parser/src/types/Enums';
...
export const UserMessage = observer(() => {
	...
	const displayContent = message.isE2E && !message.decryptedContent 
		? '*Decrypting message...*' 
	...
	useLayoutEffect(() => {
		if (isEditing) {
			if (!hasInitializedEditingRef.current) {
				...
				const initialValue = persistedDraft ?? displayContent;
	...
	}, [channel.id, isEditing, displayContent, message.id]);
	...
	if (message.type === MessageTypes.CLIENT_SYSTEM && message.author.id === NOCTISBOT_ID) {
		return (
			<SpoilerSyncProvider>
	...
	return (
		<SpoilerSyncProvider>
			...
			{(displayContent || isEditing) && (!shouldHideContent || isEditing) && (
				<div className={styles.messageContent}>
					{!shouldGroup && (
						<h3 className={styles.messageAuthorInfo}>
							<span className={styles.messageAuthorRow}>
			...
			<div className={styles.container}>
				{((!displayContent && !isEditing) || (shouldHideContent && !isEditing)) && !shouldGroup && (
					<h3 className={styles.messageAuthorInfo}>
						<span className={styles.messageAuthorRow}>
							<span className={styles.messageAuthorPart}>
								<MessageUsername
