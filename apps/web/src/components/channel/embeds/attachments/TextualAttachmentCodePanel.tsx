...
// Sanitizer for highlight.js output — allows only code-formatting HTML tags
const HLJS_PURIFY_CONFIG: DOMPurify.Config = {
	ALLOWED_TAGS: ['span', 'div', 'pre', 'code', 'b', 'i', 'em', 'strong'],
	ALLOWED_ATTR: ['class'],
	FORCE_BODY: true,
};
	...
	return (
		<div className={clsx(styles.previewSurface, wrapperClassName)} aria-live="polite">
			...
			>
				...
				>
					<code className="hljs" dangerouslySetInnerHTML={{__html: DOMPurify.sanitize(highlightedHtml, HLJS_PURIFY_CONFIG) as string}} />
				</pre>
			</div>
		</div>
	);
