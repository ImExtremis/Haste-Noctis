...
export const AddFriendForm: React.FC<AddFriendFormProps> = observer(({onSuccess}) => {
	...
	const getErrorMessage = () => {
		...
		if (errorCode === APIErrorCodes.NO_USERS_WITH_NOCTISTAG_EXIST) {
			return t`No user found with that NoctisTag.`;
		}
		if (errorCode === APIErrorCodes.DISCRIMINATOR_REQUIRED) {
			return t`Please enter a valid NoctisTag (Username#0000).`;
		}
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (!username || !discriminator || !/^\d{4}$/.test(discriminator)) {
			...
			setErrorCode(APIErrorCodes.NO_USERS_WITH_NOCTISTAG_EXIST);
			return;
		}
		...
		<form onSubmit={handleSubmit} className={styles.form}>
			<div className={clsx(styles.container, !isMobile && styles.containerDesktop)}>
				<Input
					...
					disabled={isLoading}
					aria-label={t`Friend's NoctisTag`}
