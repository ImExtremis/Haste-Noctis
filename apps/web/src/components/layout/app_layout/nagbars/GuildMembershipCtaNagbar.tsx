...
const NOCTIS_HQ_INVITE_CODE = 'noctis-hq';
...
export const GuildMembershipCtaNagbar = observer(({isMobile}: {isMobile: boolean}) => {
	...
	const inviteState = InviteStore.invites.get(NOCTIS_HQ_INVITE_CODE);
	const invite = inviteState?.data ?? null;

	const [isSubmitting, setIsSubmitting] = useState(false);

	useEffect(() => {
		const noctisHqGuild = GuildStore.getGuilds().find((guild) => guild.vanityURLCode === NOCTIS_HQ_INVITE_CODE);
		if (noctisHqGuild && GuildMemberStore.getMember(noctisHqGuild.id, currentUserId ?? '')) {
	...
	const handleJoinGuild = async () => {
		...
		try {
			await InviteActionCreators.fetchWithCoalescing(NOCTIS_HQ_INVITE_CODE);
		} finally {
			setIsSubmitting(false);
			ModalActionCreators.push(modal(() => <InviteAcceptModal code={NOCTIS_HQ_INVITE_CODE} />));
		}
	};
	...
	return (
		...
		>
			<NagbarContent
				isMobile={isMobile}
				onDismiss={handleDismiss}
				message={<Trans>Join Noctis HQ to chat with the team and stay up to date on the latest!</Trans>}
				actions={
					<NagbarButton isMobile={isMobile} onClick={handleJoinGuild} submitting={isSubmitting} disabled={isSubmitting}>
						<Trans>Join Noctis HQ</Trans>
					</NagbarButton>
				}
			/>
		</Nagbar>
