...
import {type ConnectionType, ConnectionTypes} from '@noctis/constants/src/ConnectionConstants';
import type {ConnectionVerificationResponse} from '@noctis/schema/src/domains/connection/ConnectionSchemas';
...
export const AddConnectionModal = observer(({defaultType}: AddConnectionModalProps) => {
	...
	const hostRecord = useMemo(
		() => (verificationData?.id ? `_noctis.${verificationData.id}` : ''),
		[verificationData?.id],
	);
	const dnsValue = useMemo(
		() => (verificationData?.token ? `noctis-verification=${verificationData.token}` : ''),
		[verificationData?.token],
	);
	const dnsUrl = useMemo(
		() => (verificationData?.id ? `https://${verificationData.id}/.well-known/noctis-verification` : ''),
		[verificationData?.id],
	);
	...
	const downloadTokenFile = useCallback(() => {
		...
		link.href = blobUrl;
		link.download = 'noctis-verification';
							...
							<div className={styles.tokenCard}>
								<div className={styles.tokenCardHeader}>
									...
									<p className={styles.tokenSubtitle}>
										<Trans>
											Download <code className={styles.inlineCode}>noctis-verification</code> and place it in your{' '}
								...
								<div className={styles.tokenDownloadRow}>
									...
									>
										<Trans>Download noctis-verification</Trans>
									</Button>
								</div>
