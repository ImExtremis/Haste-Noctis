	...
	const handleRedeem = async () => {
		...
		try {
			...
			ToastActionCreators.createToast({
				type: 'success',
				children: <Trans>Gift redeemed successfully! Enjoy your Stellar!</Trans>,
			});
			onRedeemSuccess();
		} catch (error) {
	...
	if (isUnclaimed) {
		return (
			<StatusSlate
				...
				description={<Trans>Claim your account to redeem or manage Stellar gift codes.</Trans>}
				actions={[
					{
		...
		<div className={styles.container}>
			<div>
				...
				<p className={styles.sectionDescription}>
					<Trans>Enter a gift code to redeem Stellar for your account.</Trans>
				</p>
			...
			<div>
				...
				<p className={styles.sectionDescriptionNoMargin}>
					<Trans>
						Manage your purchased Stellar gift codes. Share the gift URL with someone special or redeem it for
						yourself!
					</Trans>
				</p>
			</div>
			...
			{!loading && !error && gifts.length === 0 && (
				<StatusSlate
					...
					description={<Trans>Purchase a Stellar gift from the Stellar tab to share with friends!</Trans>}
					actions={[
						{
							text: <Trans>Go to Stellar</Trans>,
							onClick: () => ComponentDispatch.dispatch('USER_SETTINGS_TAB_SELECT', {tab: 'stellar'}),
							variant: 'primary',
							fitContent: true,
						},
					]}
