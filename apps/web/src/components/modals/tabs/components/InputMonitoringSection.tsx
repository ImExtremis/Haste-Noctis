...
export const InputMonitoringSection: React.FC = observer(() => {
	...
	const handleRequest = async () => {
		...
		if (result === 'granted') {
			...
			ToastActionCreators.error(t`Please enable Noctis in System Settings → Privacy & Security → Input Monitoring.`);
		}
	};
		...
		<div className={styles.permissionCard}>
			<div className={styles.permissionText}>
				...
				<p className={styles.permissionDescription}>
					<Trans>
						Noctis needs Input Monitoring permission to keep push-to-talk and global shortcuts working while the window
						is in the background.
					</Trans>
				</p>
				{status === 'denied' ? (
					<p className={styles.permissionHelp}>
						<Trans>Click "Open Settings" to open System Settings, then enable Noctis in Input Monitoring.</Trans>
					</p>
				) : null}
			</div>
