...
import {NoctisTagChangeModal} from '@app/components/modals/NoctisTagChangeModal';
	...
	return (
		<div>
			...
			<div className={styles.actions}>
				{!isClaimed ? (
					<Tooltip text={t(msg`Claim your account to change your NoctisTag`)}>
						<div>
							<Button variant="primary" small disabled>
								<Trans>Change NoctisTag</Trans>
							</Button>
						</div>
					</Tooltip>
				) : (
					<Button
						variant="primary"
						small
						onClick={() => ModalActionCreators.push(modal(() => <NoctisTagChangeModal user={user} />))}
					>
						<Trans>Change NoctisTag</Trans>
					</Button>
				)}
				...
				{!hasCustomDiscriminator && shouldShowPremiumFeatures() && (
					<Tooltip text={t(msg`Customize your 4-digit tag (#${user.discriminator}) to your liking with Stellar`)}>
						<button
							type="button"
							onClick={() => {
							...
							aria-label={t(msg`Get Stellar to customize your tag`)}
						>
