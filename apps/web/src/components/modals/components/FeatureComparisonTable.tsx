...
import {formatNumber} from '@noctis/number_utils/src/NumberFormatting';
...
export const FeatureComparisonTable = observer(() => {
	...
	const availablePerks = useMemo(() => STELLAR_PERKS.filter((perk) => perk.status === 'available'), []);

	const formatPerkValue = (perk: StellarPerk, value: number, isPremium: boolean): string => {
	...
	const renderPerkRow = (perk: StellarPerk) => {
		const label = perkLabels[perk.i18nKey as keyof typeof perkLabels] || perk.i18nKey;

		if (isBooleanPerk(perk)) {
			return (
				<ComparisonCheckRow key={perk.id} feature={label} freeHas={perk.freeValue} stellarHas={perk.stellarValue} />
			);
		}
		...
		if (isNumericPerk(perk)) {
			return (
				<ComparisonRow
					...
					stellarValue={formatPerkValue(perk, perk.stellarValue, true)}
				/>
			);
		}
		...
		if (isTextPerk(perk)) {
			...
			const premiumLabel =
				perkLabels[perk.stellarValueI18nKey as keyof typeof perkLabels] || perk.stellarValueI18nKey;
			return <ComparisonRow key={perk.id} feature={label} freeValue={freeLabel} stellarValue={premiumLabel} />;
		}
		...
		<div className={styles.table}>
			<div className={styles.header}>
				...
				<div className={styles.headerValues}>
					...
					<div className={styles.headerStellar}>
						<Trans>Stellar</Trans>
					</div>
				</div>
			</div>
