...
import styles from '@app/components/modals/NoctisTagChangeModal.module.css';
...
interface NoctisTagChangeModalProps {
	user: UserRecord;
}

export const NoctisTagChangeModal = observer(({user}: NoctisTagChangeModalProps) => {
	const {t} = useLingui();
	...
	const onSubmit = useCallback(
		async (data: FormInputs) => {
			...
			if (!hasCustomDiscriminator && !skipAvailabilityCheckRef.current && !confirmedRerollRef.current) {
				const tagTaken = await UserActionCreators.checkNoctisTagAvailability({
					username: usernameValue,
					discriminator: normalizedDiscriminator,
				});

				if (tagTaken && !isSameTag) {
					const noctisTag = `${usernameValue}#${normalizedDiscriminator}`;

					ModalActionCreators.push(
						modal(() => (
							<ConfirmModal
								title={t`NoctisTag already taken`}
								description={
									<div className={styles.confirmDescription}>
										<p>
											<Trans>
												The NoctisTag <strong>{noctisTag}</strong> is already taken. Continuing will reroll your
			...
			ToastActionCreators.createToast({type: 'success', children: t`NoctisTag updated`});
		},
	...
	return (
		<Modal.Root size="small" centered initialFocusRef={usernameRef}>
			<Form form={form} onSubmit={handleSubmit} aria-label={t`Change NoctisTag form`}>
				<Modal.Header title={t`Change Your NoctisTag`} />
				<Modal.Content>
					<Modal.ContentLayout>
						<Modal.Description>
							{hasCustomDiscriminator ? (
						...
						<div className={styles.noctisTagContainer}>
							<span className={styles.noctisTagLabel}>{t`NoctisTag`}</span>
							<div className={styles.noctisTagInputRow}>
								...
								<div className={styles.discriminatorInput}>
									{!hasCustomDiscriminator ? (
										showPremium ? (
											<Tooltip text={t`Get Stellar to customize your tag or keep it when changing your username`}>
							...
							{!hasCustomDiscriminator && (
								<StellarUpsell className={styles.premiumUpsell}>
