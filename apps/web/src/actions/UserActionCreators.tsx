...
import type {HarvestStatusResponse} from '@noctis/schema/src/domains/user/UserHarvestSchemas';
import type {UserPrivate} from '@noctis/schema/src/domains/user/UserResponseSchemas';
import type {PublicKeyCredentialCreationOptionsJSON, RegistrationResponseJSON} from '@simplewebauthn/browser';

const logger = new Logger('User');

interface NoctisTagAvailabilityResponse {
	taken: boolean;
}
...
export async function checkNoctisTagAvailability({
	username,
	discriminator,
}: {
	username: string;
	...
	try {
		logger.debug(`Checking availability for NoctisTag ${username}#${discriminator}`);
		const response = await http.get<NoctisTagAvailabilityResponse>({
			url: Endpoints.USER_CHECK_TAG,
			query: {username, discriminator},
		});
		return response.body.taken;
	} catch (error) {
		logger.error('Failed to check NoctisTag availability:', error);
		throw error;
	}
}
...
/**
 * Upload the user's E2E public key to the server.
 * Called after key generation on registration or first login.
 */
export async function uploadE2EPublicKey(publicKeyBase64: string): Promise<void> {
	try {
		logger.debug('Uploading E2E public key');
		await http.put({url: Endpoints.USER_E2E_PUBLIC_KEY, body: {public_key: publicKeyBase64}});
		logger.info('E2E public key uploaded successfully');
	} catch (error) {
		logger.error('Failed to upload E2E public key:', error);
		throw error;
	}
}
