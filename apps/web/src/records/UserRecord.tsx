...
import {PublicUserFlags} from '@noctis/constants/src/UserConstants';
import type {RequiredAction, User, UserPartial, UserPrivate} from '@noctis/schema/src/domains/user/UserResponseSchemas';
...
export class UserRecord {
	...
	readonly e2ePublicKey?: string | null;
	...
	constructor(user: User, options?: UserRecordOptions) {
		...
		this.e2ePublicKey = user.e2e_public_key ?? null;
	...
	withUpdates(updates: Partial<User>, options?: {clearMissingOptionalFields?: boolean}): UserRecord {
		...
		const baseFields: UserPartial = {
			...
			e2e_public_key: updates.e2e_public_key ?? this.e2ePublicKey,
		};
	...
	toJSON(): User {
		...
		const baseFields: UserPartial = {
			...
			system: this.system,
			flags: this.flags,
			e2e_public_key: this.e2ePublicKey,
		};
