...
import {MessageFlags, MessageStates, MessageTypes} from '@noctis/constants/src/ChannelConstants';
import type {MessageEmbed} from '@noctis/schema/src/domains/message/EmbedSchemas';
import type {
	AllowedMentions,
	ChannelMention,
	Message,
...
export class MessageRecord {
	...
	constructor(message: Message, options?: MessageRecordOptions) {
		...
		this.decryptedContent = (message as any).decryptedContent;
		this.isE2E = (message as any).isE2E ?? this.content.startsWith('E2E:{');
		...
		return new MessageRecord(
			{
				...
				decryptedContent: (updates as any).decryptedContent ?? this.decryptedContent,
				isE2E: (updates as any).isE2E ?? this.isE2E,
	...
	equals(other: MessageRecord): boolean {
		...
		if (this.decryptedContent !== other.decryptedContent) return false;
		if (this.isE2E !== other.isE2E) return false;
	...
	toJSON(): Message {
		return {
			...
			...(this.decryptedContent ? {decryptedContent: this.decryptedContent} : {}),
			...(this.isE2E ? {isE2E: this.isE2E} : {}),
			timestamp: this.timestamp.toISOString(),
