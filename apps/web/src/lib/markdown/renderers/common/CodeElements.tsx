...
import type {CodeBlockNode, InlineCodeNode} from '@noctis/markdown_parser/src/types/Nodes';
...
// DOMPurify config for syntax-highlighted code: allow only formatting elements
// that highlight.js and KaTeX actually emit. Never allow script / event attrs.
const CODE_PURIFY_CONFIG = DOMPurify.setConfig({
	ALLOWED_TAGS: [
		'span', 'div', 'pre', 'code', 'b', 'i', 'em', 'strong',
		// KaTeX-specific elements
		'math', 'semantics', 'mrow', 'mi', 'mn', 'mo', 'mfrac', 'msqrt',
		'msup', 'msub', 'msubsup', 'munder', 'mover', 'munderover',
		'annotation', 'svg', 'path', 'g', 'use', 'defs', 'rect', 'line',
	],
	ALLOWED_ATTR: ['class', 'style', 'aria-hidden', 'viewBox', 'focusable', 'd', 'fill', 'stroke', 'xmlns'],
	FORCE_BODY: true,
	RETURN_DOM_IMPORT: false,
});

function sanitizeHTML(html: string): string {
	return DOMPurify.sanitize(html, CODE_PURIFY_CONFIG) as string;
}
...
export const CodeBlockRenderer = observer(function CodeBlockRenderer({
			...
			return (
				<div key={id} className={markupStyles.latexCodeBlock}>
					<div className={markupStyles.codeContainer}>
						{copyButton}
						<div className={markupStyles.latexContent} dangerouslySetInnerHTML={{__html: sanitizeHTML(html)}} />
					</div>
				</div>
			);
		} catch (error) {
	...
	if (language && highlight.getLanguage(language)) {
		try {
			...
			highlightedContent = (
				<code className={clsx(markupStyles.hljs, language)} dangerouslySetInnerHTML={{__html: sanitizeHTML(highlighted.value)}} />
			);
		} catch (error) {
