...
import {DEFAULT_API_VERSION} from '@noctis/constants/src/AppConstants';

const SUDO_MODE_HEADER = 'x-noctis-sudo-mode-jwt';
...
export class HttpClient {
	...
	private async executeViaRelay<T>(method: HttpMethod, config: HttpRequestConfig): Promise<HttpResponse<T>> {
		...
		const sudoToken = this.sudoTokenProvider?.();
		if (sudoToken) {
			headers['x-noctis-sudo-mode-jwt'] = sudoToken;
		}
