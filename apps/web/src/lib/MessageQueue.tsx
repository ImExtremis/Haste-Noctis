...
class MessageQueue extends Queue<MessageQueuePayload, HttpResponse<Message> | undefined> {
	...
	private async handleSend(
		...
		if (channel && channel.isDM() && finalContent.trim().length > 0) {
			const recipientId = channel.getRecipientId();
			if (recipientId) {
				const recipient = UserStore.getUser(recipientId);
				if (recipient && recipient.e2ePublicKey) {
					try {
						const encrypted = await E2EEncryption.encryptForRecipient(finalContent, recipient.e2ePublicKey);
						finalContent = 'E2E:' + JSON.stringify(encrypted);
						logger.debug(`Encrypted message for recipient ${recipientId}`);
					} catch (e) {
						logger.error('Failed to encrypt message for DM:', e);
						// Fallback to cleartext or throw error? We should probably fail the send
						const err = new Error('E2E Encryption Failed');
						this.handleSendError(channelId, nonce, err as HttpError, i18n, payload.hasAttachments);
						completed(null, undefined, err);
						return;
					}
				}
			}
