...
import {FAVORITES_GUILD_ID, ME} from '@noctis/constants/src/AppConstants';
import {MessageStates} from '@noctis/constants/src/ChannelConstants';
import {MAX_MESSAGES_PER_CHANNEL} from '@noctis/constants/src/LimitConstants';
import type {ChannelId} from '@noctis/schema/src/branded/WireIds';
import type {GuildMemberData} from '@noctis/schema/src/domains/guild/GuildMemberSchemas';
import type {Message} from '@noctis/schema/src/domains/message/MessageResponseSchemas';
import {action, makeAutoObservable, reaction} from 'mobx';
import {getActivePrivateKey} from '@app/lib/E2EKeyStorage';
import * as E2EEncryption from '@app/lib/E2EEncryption';
...
class MessageStore {
	...
	private attemptE2EDecryptions(channelId: string, messages: Array<MessageRecord>): void {
		const encryptedMessages = messages.filter((m) => m.isE2E && !m.decryptedContent);
		if (encryptedMessages.length === 0) return;

		const privateKey = getActivePrivateKey();
		if (!privateKey) {
			logger.debug('Skipping E2E decryption. No active private key found.');
			return;
		}

		encryptedMessages.forEach((msg) => {
			try {
				const payloadStr = msg.content.substring(4);
				// eslint-disable-next-line @typescript-eslint/consistent-type-assertions
				const payload = JSON.parse(payloadStr) as E2EEncryption.EncryptedPayload;
				E2EEncryption.decryptFromSenderToString(payload, privateKey)
					.then((decrypted) => {
						// Inside then block, must run in action or call another action
						this.applyDecryptedContent(channelId, msg.id, decrypted);
					})
					.catch((e) => {
						logger.error(`Failed to decrypt E2E message ${msg.id}:`, e);
					});
			} catch (e) {
				logger.error(`Failed to parse E2E payload for message ${msg.id}:`, e);
			}
		});
	}

	@action
	private applyDecryptedContent(channelId: string, messageId: string, decryptedContent: string): void {
		...
		const updated = existing.update(messageId, (msg) => {
			return msg.withUpdates({decryptedContent} as any);
		});
	...
	handleLoadMessagesSuccessCached(action: {
		...
		ChannelMessages.commit(messages);
		this.notifyChange();
		this.attemptE2EDecryptions(action.channelId, messages.toArray());
		return false;
	}
	...
	handleIncomingMessage(action: {channelId: string; message: Message}): boolean {
		...
		ChannelMessages.commit(updated);
		this.notifyChange();
		this.attemptE2EDecryptions(action.channelId, [updated.get(action.message.id)!].filter(Boolean));
		return false;
	}
	...
	handleMessageUpdate(action: {message: Message}): boolean {
		...
		ChannelMessages.commit(updated);
		this.notifyChange();
		this.attemptE2EDecryptions(channelId, [updated.get(messageId)!].filter(Boolean));
		return true;
	}
	...
	handleMessagePreload(action: {messages: Record<ChannelId, Message>}): boolean {
		...
		if (hasChanges) {
			this.notifyChange();
			for (const channelId of Object.keys(action.messages)) {
				this.attemptE2EDecryptions(channelId, ChannelMessages.getOrCreate(channelId).toArray());
			}
		}
