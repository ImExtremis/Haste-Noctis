		...
		if (!hasGlobalExpressions) {
			if (!RuntimeConfigStore.isSelfHosted()) {
				return {
					...
					isLockedByPermission: false,
					lockReason: i18n._(msg`Unlock custom emojis in DMs with Stellar`),
				};
			}
			return {
				canUse: false,
	...
	if (!hasGlobalExpressions) {
		if (!RuntimeConfigStore.isSelfHosted()) {
			return {
				...
				isLockedByPremium: true,
				isLockedByPermission: false,
				lockReason: i18n._(msg`Unlock external custom emojis with Stellar`),
			};
		}
		...
		if (!hasGlobalExpressions) {
			if (!RuntimeConfigStore.isSelfHosted()) {
				return {
					...
					isLockedByPremium: true,
					isLockedByPermission: false,
					lockReason: i18n._(msg`Unlock stickers in DMs with Stellar`),
				};
			}
			return {
				canUse: false,
	...
	if (!hasGlobalExpressions) {
		if (!RuntimeConfigStore.isSelfHosted()) {
			return {
				...
				isLockedByPremium: true,
				isLockedByPermission: false,
				lockReason: i18n._(msg`Unlock external stickers with Stellar`),
			};
		}
		return {
			canUse: false,
