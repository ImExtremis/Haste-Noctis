...
pnpm --filter @noctis/config generate

if [ ! -f "$NOCTIS_CONFIG" ]; then
	info "Creating config from development template..."
	cp "$REPO_ROOT/config/config.dev.template.json" "$NOCTIS_CONFIG"
fi
...
jq '
	...
	.integrations.search.api_key = "noctis-devcontainer-meili-master-key" |
	...
	.integrations.voice.api_key = "noctis-devcontainer-key" |
	.integrations.voice.api_secret = "noctis-devcontainer-secret-key-00000000" |
...
' "$NOCTIS_CONFIG" > "$NOCTIS_CONFIG.tmp" && mv "$NOCTIS_CONFIG.tmp" "$NOCTIS_CONFIG"

info "Running bootstrap..."
"$REPO_ROOT/scripts/dev_bootstrap.sh"

info "Pre-compiling Erlang gateway dependencies..."
(cd "$REPO_ROOT/noctis_gateway" && rebar3 compile) || {
