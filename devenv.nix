{ pkgs, config, lib, ... }:
{
	...
	env = {
		NOCTIS_CONFIG = "${config.git.root}/config/config.json";
		NOCTIS_DATABASE = "sqlite";
		PC_DISABLE_TUI = "1";
	};
		...
		settings = {
			...
			processes = {
				...
				css_watch = {
					...
					noctis_app = {
						command = lib.mkForce "exec ${config.git.root}/scripts/dev_process_entry.sh noctis_app env FORCE_COLOR=1 NOCTIS_APP_DEV_PORT=49427 ${config.git.root}/scripts/dev_noctis_app.sh";
						log_location = "${config.git.root}/dev/logs/noctis_app.log";
							availability = {
								restart = "always";
							};
					};
				noctis_gateway = {
					command = lib.mkForce "exec ${config.git.root}/scripts/dev_process_entry.sh noctis_gateway env NOCTIS_GATEWAY_NO_SHELL=1 ${config.git.root}/scripts/dev_gateway.sh";
					log_location = "${config.git.root}/dev/logs/noctis_gateway.log";
					log_configuration = {
						flush_each_line = true;
					};
						availability = {
				...
				noctis_server = {
					command = lib.mkForce "exec ${config.git.root}/scripts/dev_process_entry.sh noctis_server pnpm --filter noctis_server dev";
					log_location = "${config.git.root}/dev/logs/noctis_server.log";
						availability = {
							restart = "always";
						};
				};
			...
			meilisearch = {
				...
				marketing_dev = {
					command = lib.mkForce "exec ${config.git.root}/scripts/dev_process_entry.sh marketing_dev env FORCE_COLOR=1 pnpm --filter noctis_marketing dev";
	...
	processes = {
		noctis_server.exec = "cd ${config.git.root} && pnpm --filter noctis_server dev";
		noctis_app.exec = "cd ${config.git.root} && FORCE_COLOR=1 NOCTIS_APP_DEV_PORT=49427 pnpm --filter noctis_app dev";
		marketing_dev.exec = "cd ${config.git.root} && FORCE_COLOR=1 pnpm --filter noctis_marketing dev";
		css_watch.exec = "cd ${config.git.root} && ${config.git.root}/scripts/dev_css_watch.sh";
		noctis_gateway.exec = "cd ${config.git.root} && ${config.git.root}/scripts/dev_gateway.sh";
		meilisearch.exec = ''
